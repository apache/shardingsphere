...
def main() -> None:
    ...
    try:
        ...
        expected_tool_names = {"database_gateway_search_metadata", "database_gateway_execute_query"}
        ...
        write_request(
            process,
            {
                ...
                "params": {
                    ...
                    "arguments": {"database": "orders", "query": "order", "object_types": ["TABLE"]},
                },
            },
        )
        ...
        if "orders" not in search_item_names:
            raise AssertionError(f"Expected database_gateway_search_metadata to expose the orders table. Actual search items: {search_items}")
        assert process.stdin is not None
        process.stdin.close()
