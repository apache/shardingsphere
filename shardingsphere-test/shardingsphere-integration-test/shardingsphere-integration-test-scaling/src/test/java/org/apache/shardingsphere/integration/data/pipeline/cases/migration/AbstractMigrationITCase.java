...
public abstract class AbstractMigrationITCase extends BaseITCase {
    ...
    protected static final String TABLE_ORDER_COPY = "t_order_copy";
    
    protected static final String TABLE_ORDER_ITEM = "t_order_item";
    ...
    protected void createSourceOrderTable() throws SQLException {
        ...
        if (createTableOrder.contains("%s")) {
            sourceExecuteWithLog(String.format(createTableOrder, TABLE_ORDER_COPY));
        } else {
    ...
    protected void createSourceTableIndexList(final String schema) throws SQLException {
        if (DatabaseTypeUtil.isPostgreSQL(getDatabaseType())) {
            sourceExecuteWithLog(String.format("CREATE INDEX IF NOT EXISTS idx_user_id ON %s.%s ( user_id )", schema, TABLE_ORDER_COPY));
        } else if (DatabaseTypeUtil.isOpenGauss(getDatabaseType())) {
            sourceExecuteWithLog(String.format("CREATE INDEX idx_user_id ON %s.%s ( user_id )", schema, TABLE_ORDER_COPY));
        }
    }
    
    protected void createSourceCommentOnList(final String schema) throws SQLException {
        sourceExecuteWithLog(String.format("COMMENT ON COLUMN %s.%s.user_id IS 'user id'", schema, TABLE_ORDER_COPY));
    }
