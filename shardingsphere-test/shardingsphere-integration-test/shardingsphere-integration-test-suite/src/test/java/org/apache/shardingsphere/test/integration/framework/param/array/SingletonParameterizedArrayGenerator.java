...
public final class SingletonParameterizedArrayGenerator {
    ...
    public static Collection<AssertionParameterizedArray> getAssertionParameterized(final SQLCommandType sqlCommandType) {
        return new ParameterizedArrayGenerator(
                ENV.getNodeEnvironment().getAdapters(), ENV.getScenarios(), ENV.getNodeEnvironment().getDatabaseTypes()).getAssertionParameterized(sqlCommandType);
    }
    ...
    public static Collection<ParameterizedArray> getCaseParameterized(final SQLCommandType sqlCommandType) {
        return new ParameterizedArrayGenerator(ENV.getNodeEnvironment().getAdapters(), ENV.getScenarios(), ENV.getNodeEnvironment().getDatabaseTypes()).getCaseParameterized(sqlCommandType);
    }
}
