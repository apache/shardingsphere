...
alterRebuild:
    REBUILD ((PARTITION EQ_OR_ASSIGN ALL)? withRebuildIndexOptions? | (PARTITION EQ_OR_ASSIGN NUMBER)? withSinglePartitionRebuildIndexOptions?)
    ;
...
rebuildIndexOption:
    ...
    | COMPRESSION_DELAY EQ_OR_ASSIGN (ZERO | NUMBER ( MINUTES )?)
...
withReorganizeOption:
    ...
    ;
reorganizeOption:
    (LOB_COMPACTION EQ_OR_ASSIGN (ON | OFF) |  COMPRESS_ALL_ROW_GROUPS EQ_OR_ASSIGN (ON | OFF))
    ;
