...
public final class MasterSlavePreparedStatement extends AbstractMasterSlavePreparedStatementAdapter {
    ...
    public MasterSlavePreparedStatement(
        ...
        for (RouteUnit each : masterSlaveRouter.route(sql, Collections.emptyList(), true).getRouteResult().getRouteUnits()) {
            PreparedStatement preparedStatement = connection.getConnection(each.getDataSourceUnit().getActualName()).prepareStatement(sql, resultSetType, resultSetConcurrency, resultSetHoldability);
    ...
    public MasterSlavePreparedStatement(final MasterSlaveConnection connection, final String sql, final int autoGeneratedKeys) throws SQLException {
        ...
        masterSlaveRouter = new MasterSlaveRouter(connection.getRuntimeContext().getRule(), connection.getRuntimeContext().getSqlParserEngine(), 
                connection.getRuntimeContext().getProperties().<Boolean>getValue(ConfigurationPropertyKey.SQL_SHOW));
        for (RouteUnit each : masterSlaveRouter.route(sql, Collections.emptyList(), true).getRouteResult().getRouteUnits()) {
            PreparedStatement preparedStatement = connection.getConnection(each.getDataSourceUnit().getActualName()).prepareStatement(sql, autoGeneratedKeys);
            routedStatements.add(preparedStatement);
        }
    }
    ...
    public MasterSlavePreparedStatement(final MasterSlaveConnection connection, final String sql, final int[] columnIndexes) throws SQLException {
        ...
        masterSlaveRouter = new MasterSlaveRouter(connection.getRuntimeContext().getRule(), connection.getRuntimeContext().getSqlParserEngine(),
                connection.getRuntimeContext().getProperties().<Boolean>getValue(ConfigurationPropertyKey.SQL_SHOW));
        for (RouteUnit each : masterSlaveRouter.route(sql, Collections.emptyList(), true).getRouteResult().getRouteUnits()) {
            PreparedStatement preparedStatement = connection.getConnection(each.getDataSourceUnit().getActualName()).prepareStatement(sql, columnIndexes);
            routedStatements.add(preparedStatement);
        }
    }
    ...
    public MasterSlavePreparedStatement(final MasterSlaveConnection connection, final String sql, final String[] columnNames) throws SQLException {
        ...
        masterSlaveRouter = new MasterSlaveRouter(connection.getRuntimeContext().getRule(), connection.getRuntimeContext().getSqlParserEngine(),
                connection.getRuntimeContext().getProperties().<Boolean>getValue(ConfigurationPropertyKey.SQL_SHOW));
        for (RouteUnit each : masterSlaveRouter.route(sql, Collections.emptyList(), true).getRouteResult().getRouteUnits()) {
            PreparedStatement preparedStatement = connection.getConnection(each.getDataSourceUnit().getActualName()).prepareStatement(sql, columnNames);
            routedStatements.add(preparedStatement);
        }
    }
