...
public final class YamlShardingDataSourceFactory {
    ...
    public static DataSource createDataSource(final File yamlFile) throws SQLException, IOException {
        ...
        return ShardingDataSourceFactory.createDataSource(config.getDataSources(), new RuleConfigurationsYamlSwapper().swap(config.getShardingRule()), config.getProps());
    }
    ...
    public static DataSource createDataSource(final Map<String, DataSource> dataSourceMap, final File yamlFile) throws SQLException, IOException {
        ...
        return ShardingDataSourceFactory.createDataSource(
                dataSourceMap, new RuleConfigurationsYamlSwapper().swap(config.getShardingRule()), config.getProps());
    }
    ...
    public static DataSource createDataSource(final Map<String, DataSource> dataSourceMap, final byte[] yamlBytes) throws SQLException, IOException {
        YamlRootShardingConfiguration config = YamlEngine.unmarshal(yamlBytes, YamlRootShardingConfiguration.class, new YamlRootShardingConfigurationConstructor());
        return ShardingDataSourceFactory.createDataSource(dataSourceMap, new RuleConfigurationsYamlSwapper().swap(config.getShardingRule()), config.getProps());
    }
}
