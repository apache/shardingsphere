...
public final class YamlShardingDataSourceFactory {
    ...
    public static DataSource createDataSource(final File yamlFile) throws SQLException, IOException {
        ...
        return ShardingDataSourceFactory.createDataSource(
                config.getDataSources(), RuleConfigurationBuilder.buildSharding(new ShardingRuleConfigurationYamlSwapper().swap(config.getShardingRule())), config.getProps());
    }
    ...
    public static DataSource createDataSource(final Map<String, DataSource> dataSourceMap, final File yamlFile) throws SQLException, IOException {
        YamlRootShardingConfiguration config = YamlEngine.unmarshal(yamlFile, YamlRootShardingConfiguration.class, new YamlRootShardingConfigurationConstructor());
        return ShardingDataSourceFactory.createDataSource(
                dataSourceMap, RuleConfigurationBuilder.buildSharding(new ShardingRuleConfigurationYamlSwapper().swap(config.getShardingRule())), config.getProps());
    }
