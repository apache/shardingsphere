...
public final class SingleTableSQLRouterTest {
    ...
    public void assertCreateRouteContextWithSingleDataSource() {
        ...
        ShardingSphereDatabaseMetaData databaseMetaData = mockSingleDataSourceMetaData();
        RouteContext actual = new SingleTableSQLRouter().createRouteContext(createLogicSQL(), databaseMetaData, singleTableRule, new ConfigurationProperties(new Properties()));
    ...
    public void assertCreateRouteContextWithReadwriteSplittingDataSource() {
        ...
        ShardingSphereDatabaseMetaData databaseMetaData = mockReadwriteSplittingDataSourceMetaData();
        RouteContext actual = new SingleTableSQLRouter().createRouteContext(createLogicSQL(), databaseMetaData, singleTableRule, new ConfigurationProperties(new Properties()));
        List<RouteUnit> routeUnits = new ArrayList<>(actual.getRouteUnits());
        assertThat(actual.getRouteUnits().size(), is(1));
    ...
    public void assertCreateRouteContextWithMultiDataSource() {
        ...
        singleTableRule.getSingleTableDataNodes().put("t_order", Collections.singletonList(mockDataNode("ds_0")));
        ShardingSphereDatabaseMetaData databaseMetaData = mockMultiDataSourceMetaData();
        RouteContext actual = new SingleTableSQLRouter().createRouteContext(createLogicSQL(), databaseMetaData, singleTableRule, new ConfigurationProperties(new Properties()));
        List<RouteUnit> routeUnits = new ArrayList<>(actual.getRouteUnits());
        assertThat(actual.getRouteUnits().size(), is(1));
        assertThat(routeUnits.get(0).getDataSourceMapper().getLogicName(), is("ds_0"));
    ...
    private ShardingSphereDatabaseMetaData mockSingleDataSourceMetaData() {
    ...
    private ShardingSphereDatabaseMetaData mockReadwriteSplittingDataSourceMetaData() {
        ShardingSphereDatabaseMetaData result = mock(ShardingSphereDatabaseMetaData.class, RETURNS_DEEP_STUBS);
        Map<String, DataSource> dataSourceMap = new HashMap<>(2, 1);
    ...
    private ShardingSphereDatabaseMetaData mockMultiDataSourceMetaData() {
        ShardingSphereDatabaseMetaData result = mock(ShardingSphereDatabaseMetaData.class, RETURNS_DEEP_STUBS);
        Map<String, DataSource> dataSourceMap = new HashMap<>(2, 1);
        dataSourceMap.put("ds_0", mock(DataSource.class, RETURNS_DEEP_STUBS));
