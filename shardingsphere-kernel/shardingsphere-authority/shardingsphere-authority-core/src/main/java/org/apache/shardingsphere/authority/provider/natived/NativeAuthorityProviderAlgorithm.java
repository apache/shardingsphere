...
public final class NativeAuthorityProviderAlgorithm implements AuthorityProviderAlgorithm {
    ...
    @Override
    public AuthorityRegistry buildAuthorityRegistry(final Map<String, ShardingSphereDatabaseMetaData> databaseMetaDataMap, final Collection<ShardingSphereUser> users) {
        return new UserPrivilegeMapAuthorityRegistry(StoragePrivilegeBuilder.build(new LinkedList<>(databaseMetaDataMap.values()), users));
    }
