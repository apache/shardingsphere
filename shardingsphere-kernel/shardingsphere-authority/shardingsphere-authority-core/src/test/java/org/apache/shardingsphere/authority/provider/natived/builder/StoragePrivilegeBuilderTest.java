...
public final class StoragePrivilegeBuilderTest {
    ...
    public void assertBuildInCache() {
        Collection<ShardingSphereDatabaseMetaData> databaseMetaDataList = new LinkedList<>();
        Collection<ShardingSphereUser> users = new LinkedList<>();
        ShardingSphereUser root = new ShardingSphereUser("root", "", "localhost");
        users.add(root);
        Map<ShardingSphereUser, NativePrivileges> result = StoragePrivilegeBuilder.build(databaseMetaDataList, users);
    ...
    public void assertBuildPrivilegesInStorage() throws SQLException {
        ...
        ShardingSphereUser root = new ShardingSphereUser("root", "", "localhost");
        users.add(root);
        ShardingSphereDatabaseMetaData databaseMetaData = mockDatabaseMetaData(users);
        Map<ShardingSphereUser, NativePrivileges> result = StoragePrivilegeBuilder.build(Collections.singletonList(databaseMetaData), users);
        assertThat(result.size(), is(1));
    ...
    private ShardingSphereDatabaseMetaData mockDatabaseMetaData(final Collection<ShardingSphereUser> users) throws SQLException {
