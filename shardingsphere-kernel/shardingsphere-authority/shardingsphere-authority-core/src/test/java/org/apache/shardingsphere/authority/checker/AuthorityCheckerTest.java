...
public final class AuthorityCheckerTest {
    ...
    public void assertCheckSchemaByNative() throws SQLException {
        ...
        AuthorityRule rule = new AuthorityRule(ruleConfig, createDatabaseMetaDataMap(users));
        SQLChecker<AuthorityRule> sqlChecker = SQLCheckerFactory.getInstance(Collections.singleton(rule)).get(rule);
        assertTrue(sqlChecker.check("db0", new Grantee("root", "localhost"), rule));
    ...
    private Map<String, ShardingSphereDatabaseMetaData> createDatabaseMetaDataMap(final Collection<ShardingSphereUser> users) throws SQLException {
        when(databaseMetaData.getDatabase().getName()).thenReturn("db0");
        DataSource dataSource = mockDataSourceForPrivileges(users);
        when(databaseMetaData.getResource().getAllInstanceDataSources()).thenReturn(Collections.singletonList(dataSource));
        when(databaseMetaData.getRuleMetaData().getRules()).thenReturn(Collections.emptyList());
        return Collections.singletonMap("db0", databaseMetaData);
    }
