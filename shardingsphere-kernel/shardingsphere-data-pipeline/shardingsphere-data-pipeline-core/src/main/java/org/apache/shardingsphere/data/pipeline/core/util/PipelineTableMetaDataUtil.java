...
import org.apache.shardingsphere.data.pipeline.core.exception.job.SplitTableRangeException;
...
public final class PipelineTableMetaDataUtil {
    ...
    private static PipelineColumnMetaData mustGetAnAppropriateUniqueKeyColumn(final PipelineTableMetaData tableMetaData, final String tableName) {
        ShardingSpherePreconditions.checkNotNull(tableMetaData, new SplitTableRangeException(tableName, "can not get table metadata"));
        List<String> primaryKeys = tableMetaData.getPrimaryKeyColumns();
        if (1 == primaryKeys.size()) {
            return tableMetaData.getColumnMetaData(tableMetaData.getPrimaryKeyColumns().get(0));
        }
        ShardingSpherePreconditions.checkState(primaryKeys.isEmpty(), new SplitTableRangeException(tableName, "primary key is union primary"));
        Collection<PipelineIndexMetaData> uniqueIndexes = tableMetaData.getUniqueIndexes();
        ShardingSpherePreconditions.checkState(!uniqueIndexes.isEmpty(), new SplitTableRangeException(tableName, "no primary key or unique index"));
        ...
        throw new SplitTableRangeException(tableName, "table contains multiple unique index or unique index contains nullable/multiple column(s)");
    }
}
