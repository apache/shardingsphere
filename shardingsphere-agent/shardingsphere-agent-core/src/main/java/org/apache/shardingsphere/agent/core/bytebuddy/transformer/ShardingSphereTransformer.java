...
import org.apache.shardingsphere.agent.api.point.ClassStaticMethodAroundPoint;
import org.apache.shardingsphere.agent.api.point.ConstructorPoint;
import org.apache.shardingsphere.agent.api.point.InstanceMethodAroundPoint;
...
public final class ShardingSphereTransformer implements Transformer {
    ...
    public Builder<?> transform(final Builder<?> builder, final TypeDescription typeDescription, final ClassLoader classLoader, final JavaModule module) {
        if (pluginLoader.containsType(typeDescription)) {
            ...
            result = interceptorClassStaticMethodPoint(typeDescription, pluginInterceptorPoint.getClassStaticMethodAroundPoints(), result);
            result = interceptorInstanceMethodPoint(typeDescription, pluginInterceptorPoint.getInstanceMethodAroundPoints(), result);
            return result;
        }
        return builder;
    }
    ...
    private Builder<?> interceptorClassStaticMethodPoint(final TypeDescription description, final List<ClassStaticMethodAroundPoint> classStaticMethodAroundPoints, final Builder<?> builder) {
        List<ShardingSphereTransformationPoint<? extends ClassStaticMethodAroundInterceptor>> classStaticMethodAdvicePoints = description.getDeclaredMethods().stream()
                ...
                .map(md -> {
                    List<ClassStaticMethodAroundPoint> advices = classStaticMethodAroundPoints.stream()
                            .filter(point -> point.getMatcher().matches(md))
                            .collect(Collectors.toList());
                    if (advices.isEmpty()) {
                        return null;
                    ...
                    if (advices.size() == 1) {
                        ...
                        List<ClassStaticMethodAroundAdvice> collect = advices.stream()
                                .map(ClassStaticMethodAroundPoint::getAdvice)
    ...
    private Builder<?> interceptorInstanceMethodPoint(final TypeDescription description, final List<InstanceMethodAroundPoint> instanceMethodAroundPoints, final Builder<?> builder) {
        List<ShardingSphereTransformationPoint<? extends InstanceMethodAroundInterceptor>> instanceMethodAdviceComposePoints = description.getDeclaredMethods().stream()
                ...
                .map(md -> {
                    List<InstanceMethodAroundPoint> advices = instanceMethodAroundPoints.stream()
                            .filter(point -> point.getMatcher().matches(md))
                            .collect(Collectors.toList());
                    if (advices.isEmpty()) {
                        return null;
                    ...
                    if (advices.size() == 1) {
                        ...
                        List<InstanceMethodAroundAdvice> collect = advices.stream()
                                .map(InstanceMethodAroundPoint::getAdvice)
