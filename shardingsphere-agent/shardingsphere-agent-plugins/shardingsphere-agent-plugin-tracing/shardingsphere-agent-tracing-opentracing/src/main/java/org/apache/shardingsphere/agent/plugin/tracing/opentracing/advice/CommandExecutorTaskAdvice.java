...
public final class CommandExecutorTaskAdvice implements InstanceMethodAroundAdvice {
    ...
    public void beforeMethod(final AdviceTargetObject target, final Method method, final Object[] args, final MethodInvocationResult result) {
        Scope scope = GlobalTracer.get().buildSpan(OPERATION_NAME)
                ...
                .startActive(true);
        ExecutorDataMap.getValue().put(OpenTracingConstants.ROOT_SPAN, scope.span());
    }
    
    @Override
    @SneakyThrows
    public void afterMethod(final AdviceTargetObject target, final Method method, final Object[] args, final MethodInvocationResult result) {
        ExecutorDataMap.getValue().remove(OpenTracingConstants.ROOT_SPAN);
        ...
        scope.span().setTag(OpenTracingConstants.ShardingSphereTags.CONNECTION_COUNT.getKey(), connection.getConnectionSize());
        scope.close();
    }
