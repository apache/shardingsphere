...
public final class SQLParsedResultTest {
    ...
    public void assertToString() throws IOException {
        ...
        assertThat(actual.toString(), is("SQLParsedResult(routeContext=RouteContext(tables=[TableContext(originalLiterals=order, name=order, alias=Optional.of(o)), "
                ...
                + "operator=IN, values=[1, 2, 3], valueIndices=[])})], mergeContext=MergeContext(orderByColumns=[OrderByColumn(super=AbstractSortableColumn(owner=Optional.absent(), "
                + "name=Optional.of(id), alias=Optional.of(a), orderByType=DESC), index=Optional.absent(), columnIndex=0)], groupByColumns=[GroupByColumn("
