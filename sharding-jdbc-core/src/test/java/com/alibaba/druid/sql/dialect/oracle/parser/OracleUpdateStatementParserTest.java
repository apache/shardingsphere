...
public final class OracleUpdateStatementParserTest {
    ...
    public void parseStatementWithUpdateWithReturn() {
        OracleStatementParser statementParser = new OracleStatementParser(null, Collections.emptyList(), 
                "UPDATE /*+ index(field1) */ ONLY TABLE_XXX as xxx SET field1=1 WHERE field1<1 RETURN * LOG ERRORS INTO TABLE_LOG");
        ...
        assertThat(((SQLExprTableSource) updateStatement.getTableSource()).getExpr().toString(), is("TABLE_XXX"));
        ...
        assertThat(updateStatement.getSqlContext().getSqlBuilder().toString(), is(
                "UPDATE /*+ index(field1) */ ONLY [Token(TABLE_XXX)] AS xxx SET field1=1 WHERE field1<1 RETURN * LOG ERRORS INTO TABLE_LOG"));
    }
    
    @Test
    public void parseStatementWithUpdateWithReturning() {
        OracleStatementParser statementParser = new OracleStatementParser(null, Collections.emptyList(), "UPDATE /*+ index(field1) */ ONLY TABLE_XXX xxx SET field1=1 WHERE field1<1 RETURNING *");
        OracleUpdateStatement updateStatement = (OracleUpdateStatement) statementParser.parseStatement();
        assertThat(updateStatement.getDbType(), is(JdbcConstants.ORACLE));
        assertThat(updateStatement.getHints().size(), is(1));
        assertThat(((SQLCommentHint) updateStatement.getHints().get(0)).getText(), is(" index(field1) "));
        ...
        assertThat(updateStatement.getAppendices().get(1), is("*"));
        assertThat(updateStatement.getSqlContext().getSqlBuilder().toString(), is("UPDATE /*+ index(field1) */ ONLY [Token(TABLE_XXX)] AS xxx SET field1=1 WHERE field1<1 RETURNING *"));
    }
    
    @Test
    public void parseStatementWithUpdateWithLog() {
        OracleStatementParser statementParser = new OracleStatementParser(null, Collections.emptyList(), 
                "UPDATE /*+ index(field1) */ ONLY TABLE_XXX as xxx SET field1=1 WHERE field1<1 LOG ERRORS INTO TABLE_LOG");
        OracleUpdateStatement updateStatement = (OracleUpdateStatement) statementParser.parseStatement();
        assertThat(updateStatement.getDbType(), is(JdbcConstants.ORACLE));
        assertThat(updateStatement.getHints().size(), is(1));
        assertThat(((SQLCommentHint) updateStatement.getHints().get(0)).getText(), is(" index(field1) "));
        ...
        assertThat(updateStatement.getSqlContext().getSqlBuilder().toString(), is("UPDATE /*+ index(field1) */ ONLY [Token(TABLE_XXX)] AS xxx SET field1=1 WHERE field1<1 LOG ERRORS INTO TABLE_LOG"));
    }
}
