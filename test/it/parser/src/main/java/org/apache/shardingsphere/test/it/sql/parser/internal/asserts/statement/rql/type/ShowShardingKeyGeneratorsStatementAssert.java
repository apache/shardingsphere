...
public final class ShowShardingKeyGeneratorsStatementAssert {
    ...
    public static void assertIs(final SQLCaseAssertContext assertContext, final ShowShardingKeyGeneratorsStatement actual,
        ...
        if (null == expected.getName()) {
            assertFalse(actual.getName().isPresent(), assertContext.getText("Actual key generator name should not exist."));
        } else {
            assertThat(assertContext.getText("Key generator name assertion error: "), actual.getName().orElse(null), is(expected.getName()));
        }
    }
}
