...
public final class DorisRefreshMaterializedViewStatementAssert {
    ...
    public static void assertIs(final SQLCaseAssertContext assertContext, final DorisRefreshMaterializedViewStatement actual, final DorisRefreshMaterializedViewStatementTestCase expected) {
        ...
        if (null != expected.getRefreshType()) {
            assertThat(assertContext.getText("Refresh type does not match: "), actual.getRefreshType(), is(expected.getRefreshType()));
        }
        if (!expected.getPartitionNames().isEmpty()) {
            assertThat(assertContext.getText("Partition names size does not match: "), actual.getPartitionNames().size(), is(expected.getPartitionNames().size()));
            assertThat(assertContext.getText("Partition names do not match: "), actual.getPartitionNames().containsAll(expected.getPartitionNames()), is(true));
        }
    }
}
