...
public final class RulesMigrationE2EIT extends AbstractMigrationE2EIT {
    ...
    public static Collection<PipelineTestParameter> getTestParameters() {
        ...
        List<String> versions = ENV.listStorageContainerImages(new MySQLDatabaseType());
        if (versions.isEmpty()) {
            return result;
        }
    ...
    private void assertMigrationSuccess(final Callable<Void> addRuleFn) throws Exception {
        ...
        try (Connection connection = getSourceDataSource().getConnection()) {
            PipelineCaseHelper.batchInsertOrderRecordsWithGeneralColumns(connection, new UUIDKeyGenerateAlgorithm(), getSourceTableOrderName(), TABLE_INIT_ROW_COUNT);
        }
        ...
        assertThat(getTargetTableRecordsCount(getSourceTableOrderName()), is(TABLE_INIT_ROW_COUNT));
    }
}
