...
1. Call `database_gateway_search_metadata` with `{"database":"%s","schema":"%s","query":"%s","object_types":["TABLE"]}`.
2. Call `mcp_read_resource` with `{"uri":"%s"}`.
3. Call `database_gateway_execute_query` with `{"database":"%s","schema":"%s","sql":"%s","max_rows":10}`.
...
When you are asked for the final answer, return JSON only with:
{
  ...
  "query": "%s",
  "totalOrders": %d,
  "toolSequence": ["database_gateway_search_metadata", "mcp_read_resource", "database_gateway_execute_query"]
}
