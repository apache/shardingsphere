...
abstract class AbstractProductionH2RuntimeSmokeE2ETest extends AbstractProductionRuntimeE2ETest {
    ...
    void assertServiceCapabilitiesResource() throws IOException, InterruptedException {
        try (MCPInteractionClient interactionClient = createOpenedInteractionClient()) {
            assertThat(interactionClient.readResource("shardingsphere://capabilities").getStructuredContent().get("supportedTools"), is(List.of("search_metadata", "execute_query")));
        }
    }
    ...
    void assertTableDetailResource() throws IOException, InterruptedException {
        try (MCPInteractionClient interactionClient = createOpenedInteractionClient()) {
            List<Map<String, Object>> items = getPayloadItems(interactionClient.readResource("shardingsphere://databases/logic_db/schemas/public/tables/orders").getStructuredContent());
            assertThat(items.size(), is(1));
    ...
    void assertListResources() throws IOException, InterruptedException {
        try (MCPInteractionClient interactionClient = createOpenedInteractionClient()) {
            MCPInteractionResponse response = interactionClient.listResources();
            assertTrue(getResources(response).stream().anyMatch(each -> "shardingsphere://capabilities".equals(each.get("uri"))));
        }
    }
    ...
    void assertExecuteUpdate() throws SQLException, IOException, InterruptedException {
        try (MCPInteractionClient interactionClient = createOpenedInteractionClient()) {
            Map<String, Object> payload = getStructuredContent(interactionClient.call("execute_query",
    ...
    void assertReadSequencesResource() throws IOException, InterruptedException {
        try (MCPInteractionClient interactionClient = createOpenedInteractionClient()) {
            List<Map<String, Object>> items = getPayloadItems(interactionClient.readResource("shardingsphere://databases/logic_db/schemas/public/sequences").getStructuredContent());
            assertThat(items.size(), is(1));
    ...
    void assertSearchSequence() throws IOException, InterruptedException {
        try (MCPInteractionClient interactionClient = createOpenedInteractionClient()) {
            List<Map<String, Object>> items = getPayloadItems(getStructuredContent(interactionClient.call("search_metadata",
    ...
    void assertExecuteSavepointFlow() throws SQLException, IOException, InterruptedException {
        try (MCPInteractionClient interactionClient = createOpenedInteractionClient()) {
            ...
            interactionClient.call("execute_query",
                    Map.of("database", "logic_db", "schema", "public", "sql", "UPDATE orders SET status = 'PENDING' WHERE order_id = 1"));
            Map<String, Object> savepointResponse = getStructuredContent(interactionClient.call("execute_query",
            ...
            Map<String, Object> rollbackResponse = getStructuredContent(interactionClient.call("execute_query",
                    Map.of("database", "logic_db", "schema", "public", "sql", "ROLLBACK TO SAVEPOINT sp_1")));
            Map<String, Object> commitResponse = getStructuredContent(interactionClient.call("execute_query",
