...
public final class LLMUsabilityScenarioCatalog {
    ...
    public List<LLMUsabilityScenario> createCoreGate(final String runtimeKind, final String databaseName, final String schemaName, final String tableName,
        ...
        result.add(createScenario("natural-mask-rule-md5-" + runtimeKind, LLMUsabilityDimension.TOOL, runtimeKind,
                ...
                List.of(MCPInteractionActionNames.READ_RESOURCE, "database_gateway_plan_mask_rule"), List.of(), false, false));
        result.add(createScenario("natural-encrypt-rule-aes-" + runtimeKind, LLMUsabilityDimension.TOOL, runtimeKind,
                List.of(LLMUsabilityScenario.NATURAL_TASK_TAG, "natural", "workflow", "encrypt"),
                new LLME2EScenario("natural-encrypt-rule-aes-" + runtimeKind, SYSTEM_PROMPT,
                        "A user asks you to identify and create an encrypt rule now for logical database `" + databaseName + "`, schema `" + schemaName + "`, table `" + tableName
                                + "`, and column `status` using AES with cipher column `status_cipher`. Use a protected secret placeholder for `aes-key-value`, "
