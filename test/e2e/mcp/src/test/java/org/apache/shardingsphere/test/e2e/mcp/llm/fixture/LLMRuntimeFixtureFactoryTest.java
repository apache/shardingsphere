...
class LLMRuntimeFixtureFactoryTest {
    ...
    void assertDockerUnavailableFails() {
        try (MockedStatic<MySQLRuntimeTestSupport> mocked = mockStatic(MySQLRuntimeTestSupport.class)) {
            ...
            mocked.when(() -> MySQLRuntimeTestSupport.createDockerRequiredMessage("Docker is required."))
                    .thenReturn("Docker is required.");
            IllegalStateException actual = assertThrows(IllegalStateException.class,
                    () -> new LLMRuntimeFixtureFactory().createMySQLFixture("logic_db", "Docker is required."));
