...
public final class LLMMCPConversationRunner {
    ...
    private List<String> createTurnToolNames(final LLME2EScenario scenario, final List<MCPInteractionTraceRecord> interactionTrace) {
        ...
        if (hasApprovalRequiredNextAction(interactionTrace)) {
    ...
    private LLME2EArtifactBundle processToolCall(final LLME2EScenario scenario, final LLMToolCall toolCall, final List<LLMChatMessage> messages,
        ...
        try {
            ...
            final String toolName = normalizeToolName(toolCall.getName(), rawArguments, artifacts.getInteractionTrace(), currentToolNames);
    ...
    private String normalizeToolName(final String toolName, final Map<String, Object> rawArguments, final List<MCPInteractionTraceRecord> interactionTrace,
                                     final List<String> turnToolNames) {
        return shouldRouteReadOnlySqlToOfferedQueryTool(toolName, rawArguments, interactionTrace, turnToolNames)
                ? "database_gateway_execute_query"
                : toolName;
    }
    
    private boolean shouldRouteReadOnlySqlToOfferedQueryTool(final String toolName, final Map<String, Object> rawArguments,
                                                             final List<MCPInteractionTraceRecord> interactionTrace, final List<String> turnToolNames) {
        return hasApprovalRequiredNextAction(interactionTrace)
                && !turnToolNames.contains(toolName)
    ...
    private Map<String, Object> normalizeToolNameArguments(final String rawToolName, final String toolName, final Map<String, Object> rawArguments) {
        ...
        result.remove("approved_by_user");
        return result;
    }
    ...
    private String createModelFacingToolResponse(final Map<String, Object> response) {
        ...
        copyIfPresent(response, result, "requires_user_approval");
    ...
    private void copyModelFacingNextActions(final Map<String, Object> source, final Map<String, Object> target) {
        ...
        for (Map<String, Object> each : nextActions) {
            result.add(Boolean.TRUE.equals(each.get("requires_user_approval")) ? createApprovalRequiredNextActionSummary(each) : each);
        }
        target.put("next_actions", result);
    }
    
    private Map<String, Object> createApprovalRequiredNextActionSummary(final Map<String, Object> action) {
        final Map<String, Object> result = new LinkedHashMap<>(4, 1F);
        copyIfPresent(action, result, "type");
        copyIfPresent(action, result, "title");
        copyIfPresent(action, result, "requires_user_approval");
    ...
    private void addTraceDrivenInstruction(final LLME2EScenario scenario, final List<LLMChatMessage> messages, final List<MCPInteractionTraceRecord> interactionTrace) {
        ...
        if (hasApprovalRequiredNextAction(interactionTrace)) {
            messages.add(LLMChatMessage.user(createApprovalRequiredNextActionInstruction()));
            return;
        }
    }
    ...
    private String createRequiredToolCallInstruction(final LLME2EScenario scenario, final LLMMCPConversationArtifacts artifacts) {
        ...
        final String approvalInstruction = hasApprovalRequiredNextAction(artifacts.getInteractionTrace()) ? " " + createApprovalRequiredNextActionInstruction() : "";
        return String.format(Locale.ENGLISH,
                "Required MCP tool coverage is incomplete. Remaining required MCP tools: %s. "
                        ...
                        + "If database_gateway_execute_query is remaining, set database `%s`, schema `%s`, and sql `%s`.%s%s%s%s%s",
                ...
                planningInstruction, workflowPlanInstruction, approvalInstruction);
    }
    
    private String createApprovalRequiredNextActionInstruction() {
        return "The latest MCP response contains approval-required next_actions; do not execute them in this score lane. Continue only with remaining read-only verification.";
    }
    
    private boolean hasApprovalRequiredNextAction(final List<MCPInteractionTraceRecord> interactionTrace) {
        return !interactionTrace.isEmpty() && LLMMCPNextActions.getNextActions(interactionTrace.getLast().getStructuredContent()).stream()
                .anyMatch(each -> Boolean.TRUE.equals(each.get("requires_user_approval")));
    }
