...
@Testcontainers
class SeataWithShardingTableTest {
    ...
    private final PostgreSQLContainer<?> postgresContainer = new PostgreSQLContainer<>("postgres:17.2-bookworm")
            .withCopyFileToContainer(
                    MountableFile.forHostPath(Paths.get("src/test/resources/test-native/sh/seata-tables-postgres.sh").toAbsolutePath()),
                    "/docker-entrypoint-initdb.d/seata-tables-postgres.sh");
    ...
    void assertShardingInLocalTransactions() throws SQLException {
        ...
        try (
                ...
                Statement statement = connection.createStatement()) {
            statement.execute("REGISTER STORAGE UNIT ds (\n"
                    + "  URL='jdbc:postgresql://127.0.0.1:" + postgresContainer.getMappedPort(5432) + "/demo_ds',\n"
                    + "  USER='test',\n"
                    + "  PASSWORD='test'\n"
                    + ")");
            statement.execute("CREATE SHARDING TABLE RULE t_order (\n"
                    + "    DATANODES('<LITERAL>ds.t_order_0, ds.t_order_1, ds.t_order_2'),\n"
                    + "    TABLE_STRATEGY(TYPE='standard', SHARDING_COLUMN=user_id, SHARDING_ALGORITHM(\n"
                    + "            TYPE(\n"
                    + "                NAME=CLASS_BASED,\n"
                    + "                PROPERTIES(\n"
                    + "                    'strategy'='STANDARD',\n"
                    + "                    'algorithmClassName'='org.apache.shardingsphere.test.natived.commons.algorithm.TOrderShardingTableAlg'\n"
                    + "                )\n"
                    + "            )\n"
                    + "        )\n"
                    + "    ),\n"
                    ...
                    + "), t_order_item (\n"
                    + "    DATANODES('<LITERAL>ds.t_order_item_0, ds.t_order_item_1, ds.t_order_item_2'),\n"
                    + "    TABLE_STRATEGY(\n"
                    + "        TYPE='standard',\n"
                    ...
                    + "                PROPERTIES(\n"
                    + "                    'strategy'='STANDARD',\n"
                    + "                    'algorithmClassName'='org.apache.shardingsphere.test.natived.commons.algorithm.TOrderItemShardingTableAlg'\n"
                    + "                )\n"
                    + "            )\n"
                    + "        )\n"
                    + "    ),\n"
