...
class EncryptInsertValueParameterRewriterTest {
    ...
    void assertRewriteWithDefaultSchema() {
        ...
        List<List<Object>> groupedParams = Collections.singletonList(Collections.singletonList("foo_value"));
        GroupedParameterBuilder paramBuilder = new GroupedParameterBuilder(groupedParams, Collections.emptyList(), Collections.emptyList());
        new EncryptInsertValueParameterRewriter(rule, database).rewrite(paramBuilder, createSQLStatementContext(groupedParams), Collections.singletonList("foo_value"));
    ...
    private InsertStatementContext createSQLStatementContext(final List<List<Object>> groupedParams) {
        ...
        when(result.getDescendingColumnNames()).thenReturn(Collections.singleton("foo_column").iterator());
        when(result.getColumnNames()).thenReturn(Collections.singletonList("foo_column"));
        when(result.getGroupedParameters()).thenReturn(groupedParams);
        ParameterMarkerExpressionSegment valueExpression = new ParameterMarkerExpressionSegment(0, 0, 0);
        InsertValueContext insertValueContext = new InsertValueContext(Collections.singletonList(valueExpression), groupedParams.get(0), 0);
