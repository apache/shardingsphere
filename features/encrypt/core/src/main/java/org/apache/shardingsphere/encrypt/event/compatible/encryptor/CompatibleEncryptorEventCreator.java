...
public final class CompatibleEncryptorEventCreator implements NamedRuleItemChangedEventCreator {
    ...
    public RuleItemChangedEvent create(final String databaseName, final String encryptorName, final DataChangedEvent event) {
        if (Type.ADDED == event.getType() || Type.UPDATED == event.getType()) {
            return new AlterNamedRuleItemEvent(databaseName, encryptorName, event.getKey(), event.getValue(), CompatibleEncryptorChangedGenerator.TYPE);
        }
        return new DropNamedRuleItemEvent(databaseName, encryptorName, CompatibleEncryptorChangedGenerator.TYPE);
    }
}
