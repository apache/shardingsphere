...
public final class CompatibleEncryptTableEventCreator implements NamedRuleItemChangedEventCreator {
    ...
    public RuleItemChangedEvent create(final String databaseName, final String groupName, final DataChangedEvent event) {
        if (Type.ADDED == event.getType() || Type.UPDATED == event.getType()) {
            return new AlterNamedRuleItemEvent(databaseName, groupName, event.getKey(), event.getValue(), CompatibleEncryptTableChangedGenerator.TYPE);
        }
        return new DropNamedRuleItemEvent(databaseName, groupName, CompatibleEncryptTableChangedGenerator.TYPE);
    }
}
