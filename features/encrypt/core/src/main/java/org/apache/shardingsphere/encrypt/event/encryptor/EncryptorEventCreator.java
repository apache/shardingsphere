...
public final class EncryptorEventCreator implements NamedRuleItemChangedEventCreator {
    ...
    public RuleItemChangedEvent create(final String databaseName, final String encryptorName, final DataChangedEvent event) {
        if (Type.ADDED == event.getType() || Type.UPDATED == event.getType()) {
            return new AlterNamedRuleItemEvent(databaseName, encryptorName, event.getKey(), event.getValue(), EncryptorChangedGenerator.TYPE);
        }
        return new DropNamedRuleItemEvent(databaseName, encryptorName, EncryptorChangedGenerator.TYPE);
    }
}
