...
public final class ReadwriteSplittingLoadBalancerEventCreator implements NamedRuleItemChangedEventCreator {
    ...
    public RuleItemChangedEvent create(final String databaseName, final String loadBalancerName, final DataChangedEvent event) {
        if (Type.ADDED == event.getType() || Type.UPDATED == event.getType()) {
            return new AlterNamedRuleItemEvent(databaseName, loadBalancerName, event.getKey(), event.getValue(), ReadwriteSplittingLoadBalancerChangedGenerator.TYPE);
        }
        return new DropNamedRuleItemEvent(databaseName, loadBalancerName, ReadwriteSplittingLoadBalancerChangedGenerator.TYPE);
    }
}
