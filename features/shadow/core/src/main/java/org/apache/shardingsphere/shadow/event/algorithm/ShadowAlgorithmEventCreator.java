...
public final class ShadowAlgorithmEventCreator implements NamedRuleItemChangedEventCreator {
    ...
    public RuleItemChangedEvent create(final String databaseName, final String algorithmName, final DataChangedEvent event) {
        if (Type.ADDED == event.getType() || Type.UPDATED == event.getType()) {
            return new AlterNamedRuleItemEvent(databaseName, algorithmName, event.getKey(), event.getValue(), ShadowAlgorithmChangedGenerator.TYPE);
        }
        return new DropNamedRuleItemEvent(databaseName, algorithmName, ShadowAlgorithmChangedGenerator.TYPE);
    }
}
