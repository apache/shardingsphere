...
public final class ShadowDataSourceEventCreator implements NamedRuleItemChangedEventCreator {
    ...
    public RuleItemChangedEvent create(final String databaseName, final String dataSourceName, final DataChangedEvent event) {
        if (Type.ADDED == event.getType() || Type.UPDATED == event.getType()) {
            return new AlterNamedRuleItemEvent(databaseName, dataSourceName, event.getKey(), event.getValue(), ShadowDataSourceChangedGenerator.TYPE);
        }
        return new DropNamedRuleItemEvent(databaseName, dataSourceName, ShadowDataSourceChangedGenerator.TYPE);
    }
}
