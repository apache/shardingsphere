...
class ShadowDataNodeBuilderTest {
    ...
    @BeforeEach
    void setUp() {
        when(dataSourceMapperRuleAttribute.getDataSourceMapper()).thenReturn(Collections.singletonMap("foo_db", Arrays.asList("foo_prod_db", "foo_shadow_db")));
        when(rule.getAttributes()).thenReturn(new RuleAttributes(dataSourceMapperRuleAttribute));
        builder = (ShadowDataNodeBuilder) OrderedSPILoader.getServices(DataNodeBuilder.class, Collections.singleton(rule)).get(rule);
    }
    
    @Test
    void assertBuild() {
        Collection<DataNode> actual = builder.build(Collections.singleton(new DataNode("foo_db.foo_tbl")), rule);
        assertThat(actual, is(Arrays.asList(new DataNode("foo_prod_db.foo_tbl"), new DataNode("foo_shadow_db.foo_tbl"))));
    }
}
