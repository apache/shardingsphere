...
/**
 * Alter sharding key generator executor.
 */
@DistSQLExecutorCurrentRuleRequired(ShardingRule.class)
@Setter
public final class AlterShardingKeyGeneratorExecutor implements DatabaseRuleAlterExecutor<AlterShardingKeyGeneratorStatement, ShardingRule, ShardingRuleConfiguration> {
    ...
    @Override
    public void checkBeforeUpdate(final AlterShardingKeyGeneratorStatement sqlStatement) {
        ...
        ShardingSpherePreconditions.checkState(rule.getConfiguration().getKeyGenerators().containsKey(sqlStatement.getName()),
                () -> new MissingRequiredRuleException("sharding key generator", database.getName(), sqlStatement.getName()));
    }
    ...
    @Override
    public Class<AlterShardingKeyGeneratorStatement> getType() {
        return AlterShardingKeyGeneratorStatement.class;
    }
}
