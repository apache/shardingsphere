...
class AlterShardingKeyGeneratorExecutorTest {
    
    private final AlterShardingKeyGeneratorExecutor executor = new AlterShardingKeyGeneratorExecutor();
    ...
    @Test
    void assertCheckBeforeUpdateWithMissingRule() {
        executor.setRule(mockRule(new ShardingRuleConfiguration()));
        AlterShardingKeyGeneratorStatement sqlStatement = new AlterShardingKeyGeneratorStatement("snowflake", new AlgorithmSegment("UUID", new Properties()));
    ...
    void assertBuildToBeAlteredRuleConfiguration() {
        ...
        assertThat(actual.getKeyGenerators().get("snowflake").getType(), is("UUID"));
    }
    ...
    void assertBuildToBeDroppedRuleConfiguration() {
        ...
        currentRuleConfig.getKeyGenerators().put("snowflake", new AlgorithmConfiguration("SNOWFLAKE", new Properties()));
        executor.setRule(mockRule(currentRuleConfig));
        ShardingRuleConfiguration actual = executor.buildToBeDroppedRuleConfiguration(new ShardingRuleConfiguration());
