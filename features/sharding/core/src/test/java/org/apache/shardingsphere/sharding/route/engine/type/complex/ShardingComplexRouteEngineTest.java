...
class ShardingComplexRouteEngineTest {
    
    @Test
    void assertRoutingForBindingTables() {
        ShardingComplexRouteEngine routeEngine = new ShardingComplexRouteEngine(ShardingRouteEngineFixtureBuilder.createShardingConditions("t_order"),
        ...
        RouteContext routeContext = routeEngine.route(ShardingRouteEngineFixtureBuilder.createBindingShardingRule());
    ...
    void assertRoutingForShardingTableJoinBroadcastTable() {
        ...
        RouteContext routeContext = routeEngine.route(ShardingRouteEngineFixtureBuilder.createBroadcastShardingRule());
        List<RouteUnit> routeUnits = new ArrayList<>(routeContext.getRouteUnits());
        assertThat(routeContext.getRouteUnits().size(), is(1));
        assertThat(routeUnits.get(0).getDataSourceMapper().getActualName(), is("ds_1"));
        assertThat(routeUnits.get(0).getTableMappers().size(), is(1));
    ...
    void assertRoutingForNonLogicTable() {
        ...
        assertThrows(ShardingTableRuleNotFoundException.class, () -> routeEngine.route(mock(ShardingRule.class)));
    }
}
