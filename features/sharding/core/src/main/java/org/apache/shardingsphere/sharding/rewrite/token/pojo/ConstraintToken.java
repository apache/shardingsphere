...
public final class ConstraintToken extends SQLToken implements Substitutable, RouteUnitAware {
    ...
    private String getConstraintValue(final RouteUnit routeUnit) {
        ...
        ((TableContextAvailable) sqlStatementContext).getTablesContext().getTableNames().stream().findFirst().map(logicAndActualTables::get).ifPresent(optional -> result.append('_').append(optional));
