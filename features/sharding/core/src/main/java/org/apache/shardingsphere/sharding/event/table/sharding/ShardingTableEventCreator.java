...
public final class ShardingTableEventCreator implements NamedRuleItemChangedEventCreator {
    ...
    public RuleItemChangedEvent create(final String databaseName, final String tableName, final DataChangedEvent event) {
        if (Type.ADDED == event.getType() || Type.UPDATED == event.getType()) {
            return new AlterNamedRuleItemEvent(databaseName, tableName, event.getKey(), event.getValue(), ShardingTableChangedGenerator.TYPE);
        }
        return new DropNamedRuleItemEvent(databaseName, tableName, ShardingTableChangedGenerator.TYPE);
    }
}
