...
public final class DefaultDatabaseShardingStrategyEventCreator implements UniqueRuleItemChangedEventCreator {
    ...
    public RuleItemChangedEvent create(final String databaseName, final DataChangedEvent event) {
        if (Type.ADDED == event.getType() || Type.UPDATED == event.getType()) {
            return new AlterUniqueRuleItemEvent(databaseName, event.getKey(), event.getValue(), DefaultDatabaseShardingStrategyChangedGenerator.TYPE);
        }
        return new DropUniqueRuleItemEvent(databaseName, DefaultDatabaseShardingStrategyChangedGenerator.TYPE);
    }
}
