...
public final class DefaultTableShardingStrategyEventCreator implements UniqueRuleItemChangedEventCreator {
    ...
    public RuleItemChangedEvent create(final String databaseName, final DataChangedEvent event) {
        if (Type.ADDED == event.getType() || Type.UPDATED == event.getType()) {
            return new AlterUniqueRuleItemEvent(databaseName, event.getKey(), event.getValue(), DefaultTableShardingStrategyChangedGenerator.TYPE);
        }
        return new DropUniqueRuleItemEvent(databaseName, DefaultTableShardingStrategyChangedGenerator.TYPE);
    }
}
