...
public final class ShardingAlgorithmEventCreator implements NamedRuleItemChangedEventCreator {
    ...
    public RuleItemChangedEvent create(final String databaseName, final String algorithmName, final DataChangedEvent event) {
        if (Type.ADDED == event.getType() || Type.UPDATED == event.getType()) {
            return new AlterNamedRuleItemEvent(databaseName, algorithmName, event.getKey(), event.getValue(), ShardingAlgorithmChangedGenerator.TYPE);
        }
        return new DropNamedRuleItemEvent(databaseName, algorithmName, ShardingAlgorithmChangedGenerator.TYPE);
    }
}
