...
public final class DefaultKeyGenerateStrategyEventCreator implements UniqueRuleItemChangedEventCreator {
    ...
    public RuleItemChangedEvent create(final String databaseName, final DataChangedEvent event) {
        if (Type.ADDED == event.getType() || Type.UPDATED == event.getType()) {
            return new AlterUniqueRuleItemEvent(databaseName, event.getKey(), event.getValue(), DefaultKeyGenerateStrategyChangedGenerator.TYPE);
        }
        return new DropUniqueRuleItemEvent(databaseName, DefaultKeyGenerateStrategyChangedGenerator.TYPE);
    }
}
