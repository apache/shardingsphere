...
public final class MaskTableCreator implements NamedRuleItemChangedEventCreator {
    ...
    public RuleItemChangedEvent create(final String databaseName, final String tableName, final DataChangedEvent event) {
        if (Type.ADDED == event.getType() || Type.UPDATED == event.getType()) {
            return new AlterNamedRuleItemEvent(databaseName, tableName, event.getKey(), event.getValue(), MaskTableChangedGenerator.TYPE);
        }
        return new DropNamedRuleItemEvent(databaseName, tableName, MaskTableChangedGenerator.TYPE);
    }
}
