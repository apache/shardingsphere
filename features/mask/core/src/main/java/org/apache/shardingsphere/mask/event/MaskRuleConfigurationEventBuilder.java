...
public final class MaskRuleConfigurationEventBuilder implements RuleConfigurationEventBuilder {
    ...
    public Optional<GovernanceEvent> build(final String databaseName, final DataChangedEvent event) {
        if (!MaskNodeConverter.getRuleRootNodePath().isRulePath(event.getKey()) || Strings.isNullOrEmpty(event.getValue())) {
            return Optional.empty();
        }
        Optional<String> tableName = MaskNodeConverter.getTableNodePath().getNameByActiveVersionPath(event.getKey());
        if (tableName.isPresent() && !Strings.isNullOrEmpty(event.getValue())) {
            return createMaskConfigEvent(databaseName, tableName.get(), event);
        }
        Optional<String> algorithmName = MaskNodeConverter.getAlgorithmNodePath().getNameByActiveVersionPath(event.getKey());
