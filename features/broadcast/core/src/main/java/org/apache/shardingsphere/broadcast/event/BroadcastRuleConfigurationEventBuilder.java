...
public final class BroadcastRuleConfigurationEventBuilder implements RuleConfigurationEventBuilder {
    ...
    public Optional<GovernanceEvent> build(final String databaseName, final DataChangedEvent event) {
        if (!BroadcastNodeConverter.getRuleRootNodePath().isRulePath(event.getKey()) || Strings.isNullOrEmpty(event.getValue())) {
            return Optional.empty();
        }
        if (BroadcastNodeConverter.getTableNodePath().getNameByActiveVersionPath(event.getKey()).isPresent() && !Strings.isNullOrEmpty(event.getValue())) {
