...
public abstract class SQLServerStatementVisitor extends SQLServerStatementBaseVisitor<ASTNode> {
    ...
    private InExpression createInSegment(final PredicateContext ctx) {
        ...
        if (null == ctx.subquery()) {
            ...
            right = new SubqueryExpressionSegment(new SubquerySegment(ctx.subquery().start.getStartIndex(), ctx.subquery().stop.getStopIndex(), (SQLServerSimpleSelectStatement) visit(ctx.subquery()),
    ...
    public final ASTNode visitSimpleExpr(final SimpleExprContext ctx) {
        ...
        if (null != ctx.subquery()) {
            return new SubquerySegment(startIndex, stopIndex, (SQLServerSimpleSelectStatement) visit(ctx.subquery()), getOriginalText(ctx.subquery()));
        }
    ...
    public ASTNode visitSelect(final SelectContext ctx) {
        SQLServerSimpleSelectStatement result = (SQLServerSimpleSelectStatement) visit(ctx.aggregationClause());
    ...
    public ASTNode visitSelectClause(final SelectClauseContext ctx) {
        SQLServerSimpleSelectStatement result = new SQLServerSimpleSelectStatement();
    ...
    private Collection<CommonTableExpressionSegment> getCommonTableExpressionSegmentsUsingCteClauseSet(final CteClauseSetContext ctx) {
        ...
        for (CteClauseContext each : ctx.cteClause()) {
            SubquerySegment subquery = new SubquerySegment(each.subquery().aggregationClause().start.getStartIndex(),
                    each.subquery().aggregationClause().stop.getStopIndex(), (SQLServerSimpleSelectStatement) visit(each.subquery()), getOriginalText(each.subquery()));
    ...
    private SubquerySegment createInsertSelectSegment(final InsertSelectClauseContext ctx) {
        SQLServerSimpleSelectStatement selectStatement = (SQLServerSimpleSelectStatement) visit(ctx.select());
    ...
    public ASTNode visitTableFactor(final TableFactorContext ctx) {
        if (null != ctx.subquery()) {
            ...
            if (null != ctx.subquery().merge()) {
                ...
                subquerySegment.setSelect((SQLServerSimpleSelectStatement) visit(ctx.subquery()));
            }
    ...
    public ASTNode visitCreateTableAsSelectClause(final CreateTableAsSelectClauseContext ctx) {
        ...
        if (null != ctx.createTableAsSelect()) {
            ...
            result.setSimpleSelectStatement((SQLServerSimpleSelectStatement) visit(ctx.createTableAsSelect().select()));
            ...
            result.setSimpleSelectStatement((SQLServerSimpleSelectStatement) visit(ctx.createRemoteTableAsSelect().select()));
        }
        return result;
    }
