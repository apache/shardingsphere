...
public abstract class SQL92StatementVisitor extends SQL92StatementBaseVisitor<ASTNode> {
    ...
    private InExpression createInSegment(final PredicateContext ctx) {
        ...
        if (null != ctx.subquery()) {
            right = new SubqueryExpressionSegment(new SubquerySegment(ctx.subquery().start.getStartIndex(), ctx.subquery().stop.getStopIndex(), (SQL92SimpleSelectStatement) visit(ctx.subquery()),
    ...
    public final ASTNode visitSimpleExpr(final SimpleExprContext ctx) {
        ...
        if (null != ctx.subquery()) {
            return new SubquerySegment(startIndex, stopIndex, (SQL92SimpleSelectStatement) visit(ctx.subquery()), getOriginalText(ctx.subquery()));
        }
