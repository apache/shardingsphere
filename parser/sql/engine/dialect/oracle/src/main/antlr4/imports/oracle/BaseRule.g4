...
dataTypeName
    ...
    | INTERVAL DAY | INTERVAL YEAR | JSON | FLOAT | REAL | DOUBLE PRECISION | INT | SMALLINT
    | DECIMAL | NUMERIC | DEC | IDENTIFIER_ | XMLTYPE | ROWID | ANYDATA | ANYTYPE | ANYDATASET
    ;

datetimeTypeSuffix
    : (WITH LOCAL? TIME ZONE)? | TO MONTH | TO SECOND (LP_ fractionalSecondPrecision RP_)?
    ;
...
terminatedBySpec
     : TERMINATED BY (STRING_ | DOUBLE_QUOTED_TEXT | WHITESPACE)
     ;

optionallyEnclosedBySpec
     : OPTIONALLY? ENCLOSED BY (STRING_ | DOUBLE_QUOTED_TEXT) (AND (STRING_ | DOUBLE_QUOTED_TEXT))?
     ;

lrtrimSpec
     : LTRIM | RTRIM | LRTRIM | LDRTRIM
     ;
