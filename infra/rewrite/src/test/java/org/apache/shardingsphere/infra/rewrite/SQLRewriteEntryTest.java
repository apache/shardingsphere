...
class SQLRewriteEntryTest {
    ...
    void assertRewriteForGenericSQLRewriteResult() {
        ...
        SQLTranslatorRule rule = new SQLTranslatorRule(new SQLTranslatorRuleConfiguration("FIXTURE", new Properties(), true));
        SQLRewriteEntry sqlRewriteEntry = new SQLRewriteEntry(database, new RuleMetaData(Collections.singleton(rule)), new ConfigurationProperties(new Properties()));
