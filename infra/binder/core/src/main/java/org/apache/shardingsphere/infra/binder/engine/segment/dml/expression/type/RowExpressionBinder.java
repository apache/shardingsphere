...
/**
 * Row expression binder.
 */
@NoArgsConstructor(access = AccessLevel.PRIVATE)
public final class RowExpressionBinder {
     ...
     * @return bound row expression
     */
    public static RowExpression bind(final RowExpression segment, final SegmentType parentSegmentType,
        ...
        RowExpression result = new RowExpression(segment.getStartIndex(), segment.getStopIndex(), segment.getText());
        for (ExpressionSegment each : segment.getItems()) {
            result.getItems().add(ExpressionSegmentBinder.bind(each, parentSegmentType, binderContext, tableBinderContexts, outerTableBinderContexts));
        }
        return result;
    }
}
