...
class StorageUnitTest {
    ...
    void assertNewWithoutUsernameAndIsNotInstanceConnectionAvailable() {
        ...
        try (MockedStatic<DatabaseTypedSPILoader> mockedLoader = mockStatic(DatabaseTypedSPILoader.class)) {
            ...
            ConnectionPropertiesParser connectionPropsParser = mock(ConnectionPropertiesParser.class);
            when(connectionPropsParser.parse("jdbc:mock://127.0.0.1/foo_ds", "", null)).thenReturn(new ConnectionProperties("127.0.0.1", 3307, null, "foo_schema", new Properties()));
    ...
    void assertNewWithUsernameAndIsInstanceConnectionAvailable() {
        ...
        try (MockedStatic<DatabaseTypedSPILoader> mockedLoader = mockStatic(DatabaseTypedSPILoader.class)) {
            ...
            ConnectionPropertiesParser connectionPropsParser = mock(ConnectionPropertiesParser.class);
            when(connectionPropsParser.parse("jdbc:mock://127.0.0.1/foo_ds", "sa", null))
                    .thenReturn(new ConnectionProperties("127.0.0.1", 3307, "foo_catalog", "foo_schema", new Properties()));
            when(connectionPropsParser.parse("jdbc:mock://127.0.0.1/foo_ds", "sa", "foo_catalog"))
                    .thenReturn(new ConnectionProperties("127.0.0.1", 3307, "foo_catalog", "foo_schema", new Properties()));
            mockedLoader.when(() -> DatabaseTypedSPILoader.getService(ConnectionPropertiesParser.class, databaseType)).thenReturn(connectionPropsParser);
