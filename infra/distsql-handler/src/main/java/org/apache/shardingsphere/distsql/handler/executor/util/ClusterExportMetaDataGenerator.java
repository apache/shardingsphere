...
public final class ClusterExportMetaDataGenerator {
    ...
    private void generateSnapshotInfo(final ShardingSphereMetaData metaData, final ExportedClusterInfo exportedClusterInfo) {
        for (ExportedSnapshotInfoGenerator each : ShardingSphereServiceLoader.getServiceInstances(ExportedSnapshotInfoGenerator.class)) {
            each.generate(metaData).ifPresent(exportedClusterInfo::setSnapshotInfo);
        }
    }
}
