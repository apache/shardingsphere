...
- 规划、执行、验证都共用统一模型
- `step-by-step` 模式可在当前服务运行期内按 `planId` 保存并恢复上下文快照
...
- 打通上游结构化意图 / 追问结果到算法候选池、推荐结果、参数模板、敏感参数打码的完整路径。
...
- 把 planning 从“算法建议”推进到“可执行工件”，同时覆盖规则生命周期变更场景。
