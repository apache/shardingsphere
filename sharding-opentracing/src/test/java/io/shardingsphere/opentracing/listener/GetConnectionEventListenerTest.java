...
public final class GetConnectionEventListenerTest extends BaseEventListenerTest {
    ...
    public void assertExecuteSuccess() {
        ...
        GetConnectionFinishEvent connectionFinishEvent = new GetConnectionFinishEvent(3, dataSourceMetaData);
        connectionFinishEvent.setExecuteSuccess();
        shardingEventBus.post(connectionFinishEvent);
    ...
    public void assertExecuteFailure() {
        shardingEventBus.post(new GetConnectionStartEvent("test_ds_name"));
        GetConnectionFinishEvent connectionFinishEvent = new GetConnectionFinishEvent(3, null);
        connectionFinishEvent.setExecuteFailure(new RuntimeException("test error"));
        shardingEventBus.post(connectionFinishEvent);
        assertThat(getTracer().finishedSpans().size(), is(1));
        MockSpan actual = getTracer().finishedSpans().get(0);
        assertThat(actual.operationName(), is("/Sharding-Sphere/getConnection/"));
