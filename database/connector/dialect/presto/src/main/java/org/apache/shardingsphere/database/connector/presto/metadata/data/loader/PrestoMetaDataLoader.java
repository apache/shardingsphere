...
public final class PrestoMetaDataLoader implements DialectMetaDataLoader {
    ...
    public Collection<SchemaMetaData> load(final MetaDataLoaderMaterial material) throws SQLException {
        ...
        try (Connection connection = material.getDataSource().getConnection()) {
            ...
            Map<String, Collection<ColumnMetaData>> columnMetaDataMap = loadColumnMetaDataMap(connection, catalogName, schemaName, material.getActualTableNames());
            Collection<String> viewNames = columnMetaDataMap.isEmpty() ? Collections.emptySet() : loadViewNames(connection, catalogName, schemaName, columnMetaDataMap.keySet());
            for (Entry<String, Collection<ColumnMetaData>> entry : columnMetaDataMap.entrySet()) {
                tableMetaDataList.add(new TableMetaData(
    ...
    private Map<String, Collection<ColumnMetaData>> loadColumnMetaDataMap(final Connection connection, final String catalogName, final String schemaName,
    ...
    private boolean isStringDataType(final String dataType) {
        ...
        return normalizedDataType.startsWith("varchar") || normalizedDataType.startsWith("char");
    }
    
    private Collection<String> loadViewNames(final Connection connection, final String catalogName, final String schemaName, final Collection<String> tableNames) throws SQLException {
