...
class MySQLSystemDatabaseTest {
    ...
    void assertGetSystemDatabases() {
        assertThat(systemDatabase.getSystemDatabases(), is(new LinkedHashSet<>(Arrays.asList("information_schema", "performance_schema", "mysql", "sys", "shardingsphere"))));
    }
    ...
    private static Stream<Arguments> getSystemSchemasByDatabaseNameArguments() {
        return Stream.of(
                Arguments.of("information schema database", "information_schema", Collections.singleton("information_schema")),
                Arguments.of("mysql database", "mysql", Collections.singleton("mysql")),
