...
| REFRESH TABLE METADATA FROM RESOURCE resourceName SCHEMA schemaName       | Refresh the tables' metadata in a schema of a specified data source. If there are no tables in the schema, the schema is deleted.                       | REFRESH TABLE METADATA FROM RESOURCE SCHEMA db_schema     |
