...
| 现象 | 可能原因 | 处理方式 |
| --- | --- | --- |
| MCP Server 启动失败 | Java 版本、配置文件路径、发行包目录或 YAML 必填字段不正确。 | 查看启动终端和 `logs/mcp.log`；确认使用 Java 21 及以上版本，配置文件存在，发行包 `lib/` 目录完整。 |
...
| 看不到数据库或逻辑库 | `runtimeDatabases` 未配置、名称不正确、连接失败、权限不足，或目标范围确实为空。 | 读取当前可见数据库；如果返回连接错误分类，按下方连接错误分类处理；确认账号拥有元数据读取权限。 |
...
| 空 `runtimeDatabases` 提示 | 未配置运行时数据库时 Server 可以启动，但元数据和 SQL 能力不可用。 | 后续在启动日志或运行状态中给出更明显的配置缺失提示。 |
