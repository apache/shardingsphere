...
| 现象                           | 可能原因                                                              | 处理方式                                                                                                 |
|------------------------------|-------------------------------------------------------------------|------------------------------------------------------------------------------------------------------|
| MCP Server 启动失败              | Java 版本、配置文件路径、发行包目录或 YAML 必填字段不正确。                               | 查看启动终端和 `logs/mcp.log`；确认使用 Java 21 及以上版本，配置文件存在，并且发行包 `lib/` 目录完整。 |
...
- ShardingSphere-MCP 不获取真实敏感值，真实敏感值也不应进入 AI 应用；如果规则变更需要密钥或凭证，应在 MCP 和 AI 应用之外的受控环境中替换人工执行包里的中性占位符。
