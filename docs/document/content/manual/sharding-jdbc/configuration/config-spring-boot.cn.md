...
spring.shardingsphere.sharding.tables.<logic-table-name>.key-generator.type= #自增列值生成算法类型，缺省表示使用默认自增列值生成算法。可使用用户自定义的列值生成算法或选择内置类型：SNOWFLAKE/UUID/LEAF_SEGMENT
...
spring.shardingsphere.sharding.default-key-generator.type= #默认自增列值生成算法类型，缺省将使用org.apache.shardingsphere.core.strategy.keygen.SnowflakeKeyGenerateAlgorithm。可使用用户自定义的列值生成算法或选择内置类型：SNOWFLAKE/UUID/LEAF_SEGMENT
