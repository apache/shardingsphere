...
spring.shardingsphere.sharding.tables.<logic-table-name>.key-generator.column= #自增列名称，缺省表示使用默认自增主键生成器的自增列名称
spring.shardingsphere.sharding.tables.<logic-table-name>.key-generator.type= #自增列值生成算法类型，缺省表示使用默认自增列值生成算法。可使用用户自定义的列值生成算法或选择内置类型：SNOWFLAKE/UUID
spring.shardingsphere.sharding.tables.<logic-table-name>.key-generator.props.<property-name>= #自增列值生成算法属性配置, 注意：使用SNOWFLAKE算法，需要配置worker.id与max.tolerate.time.difference.milliseconds属性。若使用此算法生成值作分片值，建议配置max.vibration.offset属性
...
spring.shardingsphere.sharding.default-key-generator.type= #默认自增列值生成算法类型，缺省将使用org.apache.shardingsphere.core.strategy.keygen.SnowflakeKeyGenerateAlgorithm。可使用用户自定义的列值生成算法或选择内置类型：SNOWFLAKE/UUID
spring.shardingsphere.sharding.default-key-generator.props.<property-name>= #自增列值生成算法属性配置, 比如SNOWFLAKE算法的worker.id与max.tolerate.time.difference.milliseconds
...
spring.shardingsphere.orchestration.spring_boot_ds_sharding.orchestration-type= #配置/注册/元数据中心的类型，例如config_center/registry_center/metadata_center
spring.shardingsphere.orchestration.spring_boot_ds_sharding.instance-type= #配置中心类型。如：zookeeper
...
spring.shardingsphere.orchestration.spring_boot_ds_sharding.props.overwrite=true #本地配置是否覆盖注册中心配置。如果可覆盖，每次启动都以本地配置为准
