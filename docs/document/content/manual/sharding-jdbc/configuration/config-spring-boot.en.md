...
spring.shardingsphere.sharding.tables.<logic-table-name>.key-generator.type= #Auto-increament key generate algorithm type; default means using default auto-increament key generate algorithm; user defined generate algorithm or internal generate algorithm (SNOWFLAKE, UUID, LEAF_SEGMENT) can both be selected
