...
- 通过 `next_actions` 和用户确认字段引导模型继续下一步。
...
- P0 范围锁定为 public surface、`next_actions`、metadata URI、output schema、recovery 和轻量回归保护六类问题。
- 不保留旧兼容层、旧工具名、兼容字段或隐式默认行为；实现时应直接革新到当前契约。
...
- 不为了向前兼容而保留旧推荐字段、旧工具名、旧参数默认值或旧文档契约。
...
- P0 只包含 FR-001 至 FR-006 六类问题，P1/P2 不混入第一轮实现。
- `shardingsphere://capabilities` 是当前 MCP public surface 的唯一事实源。
- 不考虑旧契约兼容性；兼容性问题应一并修正，不保留向前兼容 shim。
- 缺少 `execution_mode` 必须明确拒绝，并通过 recovery 引导到 `execution_mode=preview`。
- 验证门槛采用轻量但确定性的 descriptor lint、capabilities shape check 和 focused contract/unit tests。
...
- 当前 `next_actions`：现有字段、旧推荐字段、用户确认字段和 workflow guidance 是否已经有可复用结构。
...
其中 `shardingsphere://capabilities` 是唯一当前事实源。

验收标准：

- `shardingsphere://capabilities` 必须作为当前 MCP public surface 的唯一事实源。
...
- 缺少 `execution_mode` 时，明确拒绝当前调用，不默认 preview 或 execute。
- 缺少 `execution_mode` 的 recovery 返回 `missing_fields`，并建议先使用 `execution_mode=preview`。
...
### FR-105 审批参数与 workflow 副作用范围更明确

preview 到 execute/apply 的衔接应让模型知道哪些值可以原样复用，以及需要用户批准什么。

验收标准：

- `approved_steps` 等审批参数说明可接受的 step、artifact 或全量批准值。
- preview 响应中的可复用参数来自服务端已知值或用户已提供值。
- `apply_workflow` preview 尽量区分 rule metadata、physical structure、physical data 等副作用范围。
- 无法安全判断副作用范围时，必须保守描述，不得弱化风险。
- 不引入 approval token、preview token 或持久审批系统。

### FR-106 旧 plan 恢复路径更可执行

当 `plan_id` 不存在、过期或跨 session 时，模型应知道是补全当前 session plan，还是重新规划。

验收标准：

- recovery 明确建议使用当前 session `plan_id` completion，或重新调用对应 planning tool。
- 能保留的 workflow kind、database、schema、table、column 等上下文应作为安全参数返回。
- 不新增 current-session workflow 列表 resource。
- 不承诺跨 session 或重启后的 plan 持久化。
...
### FR-202 首次 MCP 客户端配置示例

文档应给出足够短的 STDIO 和 HTTP 客户端配置形状，帮助用户和模型接入。

验收标准：

- 覆盖本地 STDIO command 配置形状。
- 覆盖 OCI/server package 配置形状。
- 覆盖 HTTP endpoint 和 bearer token header 配置形状。
- 明确第一个建议读取的资源是 `shardingsphere://capabilities`。
- 示例不包含真实 token、密码或机器特定路径。
...
### FR-204 registry 与 server metadata 提示

`server.json` 或相邻 packaging metadata 应让首次发现 MCP 的客户端知道核心用途。

验收标准：

- 简短说明 metadata-first discovery。
- 简短说明 safe SQL preview-first。
- 简短说明 encrypt/mask workflow。
- 不加入长教程或与分支绑定的发布承诺。

### FR-205 opt-in LLM usability 场景补充
...
1. 完成已锁定的 P0：public surface、next action、search metadata URI、output schema、recovery、回归保护。
2. 再做 P1：资源导航、短示例、completion、algorithm 属性模板、审批参数、workflow 副作用范围和旧 plan 恢复。
3. 最后做 P2：启动提示、客户端配置、排障文档、registry 提示和 opt-in LLM usability 扩展。
...
- 模型以 `shardingsphere://capabilities` 作为唯一当前事实源，不依赖旧工具名或兼容字段。
