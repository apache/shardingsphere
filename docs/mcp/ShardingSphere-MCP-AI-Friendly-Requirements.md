...
- `plan_id` 不存在、过期或跨 session 时，建议查询当前 session workflow 或重新调用对应 planning tool。
...
P1 是高价值轻量增强，可以在 P0 稳定后按收益排序实现。

### FR-101 当前 session workflow 查询入口

提供当前 session 内 workflow plan 的轻量恢复入口。

验收标准：

- 能列出当前 session 的 `plan_id`、`workflow_kind`、`status`、`current_step`、`updated_at`。
- 能读取单个 plan 的状态、artifacts 摘要和推荐下一步。
- 不要求跨 session 持久化。
- 不引入复杂搜索、排序或长期记忆。
...
### FR-105 algorithm resource 暴露属性模板
...
### FR-106 元数据 freshness 提示

元数据响应应提供轻量 freshness 信息，帮助模型判断多次读取是否来自同一批上下文。

验收标准：

- 至少提供 `loaded_at` 或 `metadata_fingerprint` 之一。
- 多个 metadata resource 响应可判断是否来自同一批加载结果。
- 不要求本阶段新增主动 refresh tool。
...
敏感配置应支持从环境变量读取，避免示例诱导明文配置。

验收标准：

- `transport.http.accessToken` 可从环境变量引用。
- runtime database password 可从环境变量引用。
- 文档说明变量缺失时的启动失败或校验行为。
- 不引入完整密钥管理系统。
...
### FR-204 opt-in LLM usability 场景补充
...
2. 再做 P1：workflow 查询、资源导航、短示例、completion、algorithm 属性模板、freshness。
3. 最后做 P2：启动提示、环境变量、排障文档、opt-in LLM usability 扩展。
