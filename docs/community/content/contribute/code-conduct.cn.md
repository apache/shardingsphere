...
## 编码规范
 ...
 - 2 位及 3 位字符的专有名词缩写使用大写，超过 3 位字符的缩写则一律采用驼峰命名。
   - 2 位及 3 位字符的缩写：DBDiscoveryExampleScenario、SQL92Lexer、XMLTransfer、MySQLAdminExecutorCreator。
   - 如果是本地变量，缩写应该使用小写：dbName、mysqlAuthenticationMethod、sqlStatement。
   - 3 位字符以上的缩写应使用驼峰命名：JdbcUrlAppender、YamlAgentConfigurationSwapper。
