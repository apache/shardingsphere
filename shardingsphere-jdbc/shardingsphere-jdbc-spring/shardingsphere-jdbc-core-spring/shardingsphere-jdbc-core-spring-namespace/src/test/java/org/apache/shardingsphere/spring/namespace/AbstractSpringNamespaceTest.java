...
public abstract class AbstractSpringNamespaceTest extends AbstractJUnit4SpringContextTests {
    ...
    public void assertShardingSphereDataSource() {
        assertDataSources(getContextManager(dataSource).getMetaDataContexts().getMetaData().get(DefaultDatabase.LOGIC_NAME).getResource().getDataSources());
        assertDatabaseRules(getContextManager(dataSource).getMetaDataContexts().getMetaData().get(DefaultDatabase.LOGIC_NAME).getRuleMetaData().getRules());
