...
class SingleRuleConfigurationDecoratorTest {
    ...
    void assertDecorateLoadsTablesWhenRulesPresentButConfigEmpty() {
        ...
        try (
                ...
                MockedStatic<SingleTableDataNodeLoader> dataNodeLoader = mockStatic(SingleTableDataNodeLoader.class);
                MockedStatic<SingleTableLoadUtils> loadUtils = mockStatic(SingleTableLoadUtils.class, RETURNS_DEEP_STUBS)) {
            mockSplitAndConvert(loadUtils, Collections.singleton(SingleTableConstants.ALL_TABLES), Collections.emptyList(), Collections.emptyList(), Collections.emptyList());
            databaseTypeEngine.when(() -> DatabaseTypeEngine.getStorageType(any(DataSource.class))).thenReturn(databaseType);
            dataNodeLoader.when(() -> SingleTableDataNodeLoader.load(anyString(), anyMap(), anyCollection())).thenReturn(actualDataNodes);
    ...
    void assertDecorateLoadsAllSchemaTablesWhenSchemaSupported() {
        ...
        try (
                MockedStatic<DatabaseTypeEngine> databaseTypeEngine = mockStatic(DatabaseTypeEngine.class);
                MockedStatic<SingleTableDataNodeLoader> dataNodeLoader = mockStatic(SingleTableDataNodeLoader.class);
                MockedConstruction<DatabaseTypeRegistry> ignored = mockSchemaAwareRegistry()) {
            databaseTypeEngine.when(() -> DatabaseTypeEngine.getStorageType(any(DataSource.class))).thenReturn(databaseType);
            dataNodeLoader.when(() -> SingleTableDataNodeLoader.load(anyString(), anyMap(), anyCollection())).thenReturn(actualDataNodes);
    ...
    void assertDecorateUsesDefaultStorageTypeWhenNoDataSourceConfigured() {
        ...
        Collection<String> splitTables = new LinkedHashSet<>(Collections.singleton("*.foo_schema.t_order"));
        try (
                MockedStatic<DatabaseTypeEngine> databaseTypeEngine = mockStatic(DatabaseTypeEngine.class);
                MockedStatic<SingleTableDataNodeLoader> dataNodeLoader = mockStatic(SingleTableDataNodeLoader.class);
                MockedStatic<SingleTableLoadUtils> loadUtils = mockStatic(SingleTableLoadUtils.class, RETURNS_DEEP_STUBS);
                MockedConstruction<DatabaseTypeRegistry> ignored = mockSchemaAwareRegistry()) {
            mockSplitAndConvert(loadUtils, splitTables, configuredDataNodes, Collections.emptyList(), Collections.emptyList());
            databaseTypeEngine.when(DatabaseTypeEngine::getDefaultStorageType).thenReturn(databaseType);
            dataNodeLoader.when(() -> SingleTableDataNodeLoader.load(anyString(), anyMap(), anyCollection())).thenReturn(actualDataNodes);
            assertThat(decorator.decorate("foo_db", dataSources, Collections.singleton(mockSingleRule()), ruleConfig).getTables(), contains("foo_ds.foo_schema.t_order"));
            databaseTypeEngine.verify(DatabaseTypeEngine::getDefaultStorageType);
        }
    }
    ...
    void assertDecorateLoadsSpecifiedTablesWithExpand() {
        ...
        Collection<String> splitTables = new LinkedHashSet<>(Arrays.asList("expand_ds.*", "bar_ds.bar_tbl"));
        Collection<String> featureRequiredTables = Collections.singleton("feature_tbl");
        try (
                MockedStatic<DatabaseTypeEngine> databaseTypeEngine = mockStatic(DatabaseTypeEngine.class);
                MockedStatic<SingleTableDataNodeLoader> dataNodeLoader = mockStatic(SingleTableDataNodeLoader.class);
                MockedStatic<SingleTableLoadUtils> loadUtils = mockStatic(SingleTableLoadUtils.class, RETURNS_DEEP_STUBS)) {
            mockSplitAndConvert(loadUtils, splitTables, configuredDataNodes, Collections.emptyList(), featureRequiredTables);
            databaseTypeEngine.when(() -> DatabaseTypeEngine.getStorageType(any(DataSource.class))).thenReturn(databaseType);
            dataNodeLoader.when(() -> SingleTableDataNodeLoader.load(anyString(), anyMap(), anyCollection())).thenReturn(actualDataNodes);
    ...
    void assertDecorateThrowsWhenExpandedNodeMismatch() {
        ...
        Collection<String> splitTables = new LinkedHashSet<>(Arrays.asList("expand_ds.*", "expand_ds.t_order"));
        try (
                MockedStatic<DatabaseTypeEngine> databaseTypeEngine = mockStatic(DatabaseTypeEngine.class);
                MockedStatic<SingleTableDataNodeLoader> dataNodeLoader = mockStatic(SingleTableDataNodeLoader.class);
                MockedStatic<SingleTableLoadUtils> loadUtils = mockStatic(SingleTableLoadUtils.class, RETURNS_DEEP_STUBS)) {
    ...
    private void mockSplitAndConvert(final MockedStatic<SingleTableLoadUtils> loadUtils, final Collection<String> splitTables,
                                     final Collection<DataNode> configuredDataNodes, final Collection<String> excludedTables,
                                     final Collection<String> featureRequiredTables) {
        loadUtils.when(() -> SingleTableLoadUtils.splitTableLines(anyCollection())).thenReturn(splitTables);
        loadUtils.when(() -> SingleTableLoadUtils.getExcludedTables(anyCollection())).thenReturn(excludedTables);
        loadUtils.when(() -> SingleTableLoadUtils.convertToDataNodes(anyString(), any(DatabaseType.class), anyCollection())).thenReturn(configuredDataNodes);
        loadUtils.when(() -> SingleTableLoadUtils.getFeatureRequiredSingleTables(anyCollection())).thenReturn(featureRequiredTables);
    }
    
    private Map<String, Collection<DataNode>> createActualDataNodes(final DataNode dataNode) {
        return Collections.singletonMap(dataNode.getTableName(), new LinkedHashSet<>(Collections.singleton(dataNode)));
    }
