...
import org.apache.shardingsphere.data.pipeline.core.preparer.incremental.IncrementalTaskPreparer;
...
public final class MigrationJobPreparer {
    ...
    private void prepareIncremental(final MigrationJobItemContext jobItemContext) {
        ...
        try {
            ...
            IngestPosition position = new IncrementalTaskPreparer(databaseType).getIncrementalPosition(initIncremental, taskConfig.getDumperContext(), jobItemContext.getDataSourceManager());
    ...
    public void cleanup(final MigrationJobConfiguration jobConfig) {
        for (Entry<String, PipelineDataSourceConfiguration> entry : jobConfig.getSources().entrySet()) {
            try {
                new IncrementalTaskPreparer(entry.getValue().getDatabaseType()).destroyPosition(jobConfig.getJobId(), entry.getValue());
            } catch (final SQLException ex) {
