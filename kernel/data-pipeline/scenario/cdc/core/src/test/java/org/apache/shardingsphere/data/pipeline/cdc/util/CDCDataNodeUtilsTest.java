...
class CDCDataNodeUtilsTest {
    ...
    void assertBuildDataNodesMap() {
        ...
        DataNodeRule singleDataNodeRule = mock(DataNodeRule.class);
        when(singleDataNodeRule.getAllDataNodes()).thenReturn(Collections.singletonMap("t_order_item", Collections.singletonList(new DataNode("single.t_order_item"))));
        when(singleRule.getRuleIdentifiers()).thenReturn(new RuleIdentifiers(singleDataNodeRule));
        ...
        DataNodeRule broadcastDataNodeRule = mock(DataNodeRule.class);
        when(broadcastDataNodeRule.findFirstActualTable("t_address")).thenReturn(Optional.of("broadcast.t_address"));
        when(broadcastDataNodeRule.getAllDataNodes()).thenReturn(Collections.singletonMap("t_address", Collections.singletonList(new DataNode("broadcast.t_address"))));
        when(broadcastRule.getRuleIdentifiers()).thenReturn(new RuleIdentifiers(broadcastDataNodeRule));
