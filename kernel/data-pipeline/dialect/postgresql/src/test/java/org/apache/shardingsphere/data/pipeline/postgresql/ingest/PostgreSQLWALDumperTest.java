...
class PostgreSQLWALDumperTest {
    ...
    void setUp() {
        ...
        walDumper = new PostgreSQLWALDumper(
                dumperContext, position, channel, new StandardPipelineTableMetaDataLoader(dataSourceManager.getDataSource(dumperContext.getCommonContext().getDataSourceConfig())));
    }
    ...
    private IncrementalDumperContext createDumperContext(final String jdbcUrl, final String username, final String password) {
        ...
        commonContext.setTableNameMap(Collections.singletonMap(new ActualTableName("t_order_0"), new LogicTableName("t_order")));
