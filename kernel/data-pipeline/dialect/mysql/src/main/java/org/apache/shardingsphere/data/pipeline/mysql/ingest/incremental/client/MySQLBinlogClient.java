...
public final class MySQLBinlogClient {
    ...
    private class MySQLBinlogEventHandler extends ChannelInboundHandlerAdapter {
        ...
        @Override
        public void exceptionCaught(final ChannelHandlerContext ctx, final Throwable cause) {
            log.error("MySQLBinlogEventHandler protocol resolution error, channel: {}, lastBinlogEvent: {}", ctx.channel(), JsonEngine.toJsonString(lastBinlogEvent.get()), cause);
        ...
        private synchronized void reconnect() {
            ...
            if (continuousFailureCount.get() >= 5) {
                log.error("MySQL binlog client failed permanently, lastBinlogEvent: {}", JsonEngine.toJsonString(lastBinlogEvent.get()));
                closeChannel(true);
                return;
            }
            ...
            try {
                ...
                // CHECKSTYLE:ON
                log.error("Reconnect failed permanently, lastBinlogEvent: {}", JsonEngine.toJsonString(lastBinlogEvent.get()), ex);
                closeChannel(true);
                return;
            }
