...
public final class MultiplexMemoryPipelineChannel implements PipelineChannel {
    ...
    public void pushRecords(final List<Record> records) {
        for (Record each : records) {
            ...
            } else if (PlaceholderRecord.class.equals(each.getClass())) {
                pushRecord(each, 0);
            } else {
                throw new UnsupportedOperationException("Unsupported record type: " + each.getClass().getName());
            }
        }
    }
    ...
    private void pushRecord(final Record record, final int channelIndex) {
        ...
        channel.pushRecords(Collections.singletonList(record));
    }
