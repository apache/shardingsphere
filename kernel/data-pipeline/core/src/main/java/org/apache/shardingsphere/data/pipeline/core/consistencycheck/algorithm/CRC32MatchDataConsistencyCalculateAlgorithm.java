...
public final class CRC32MatchDataConsistencyCalculateAlgorithm extends AbstractDataConsistencyCalculateAlgorithm {
    ...
    public Iterable<DataConsistencyCalculatedResult> calculate(final DataConsistencyCalculateParameter param) {
        ...
        List<CalculatedItem> calculatedItems = param.getColumnNames().stream().map(each -> calculateCRC32(sqlBuilderEngine, param, each)).collect(Collectors.toList());
    ...
    private CalculatedItem calculateCRC32(final PipelineSQLBuilderEngine sqlBuilderEngine, final DataConsistencyCalculateParameter param, final String columnName) {
        Optional<String> sql = sqlBuilderEngine.buildCRC32SQL(param.getSchemaName(), param.getLogicTableName(), columnName);
