...
public final class CDCDataRecordUtilsTest {
    ...
    public void assertFindMinimumDataRecordAndSavePosition() throws InterruptedException {
        ...
        for (long i = 0; i <= 5; i++) {
            DataRecord minimumDataRecord = CDCDataRecordUtils.findMinimumDataRecordAndSavePosition(actualIncrementalRecordMap, dataRecordComparator, cdcAckPositionMap);
            assertThat(minimumDataRecord.getCsn(), is(i));
        }
        assertNull(CDCDataRecordUtils.findMinimumDataRecordAndSavePosition(actualIncrementalRecordMap, dataRecordComparator, cdcAckPositionMap));
    }
