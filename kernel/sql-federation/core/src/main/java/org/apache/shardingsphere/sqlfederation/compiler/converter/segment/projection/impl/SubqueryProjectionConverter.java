...
public final class SubqueryProjectionConverter implements SQLSegmentConverter<SubqueryProjectionSegment, SqlNode> {
    ...
    public Optional<SqlNode> convert(final SubqueryProjectionSegment segment) {
        ...
        return segment.getSubquery().getSubqueryType().equals(SubqueryType.EXISTS_SUBQUERY) ? Optional.of(new SqlBasicCall(SqlStdOperatorTable.EXISTS, Collections.singletonList(sqlNode), SqlParserPos.ZERO)) : Optional.of(sqlNode);
    }
