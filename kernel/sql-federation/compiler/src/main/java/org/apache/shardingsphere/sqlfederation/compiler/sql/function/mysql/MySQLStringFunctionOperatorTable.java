...
/**
 * MySQL-specific string function operator table, registered only into MySQL compiler context so that
 * {@code LENGTH} / {@code BIT_LENGTH} / {@code LOCATE} keep their MySQL byte-count and 1-based-position semantics
 * without leaking into PostgreSQL, openGauss, Oracle, or fallback contexts where the Calcite library entries (such as
 * the PostgreSQL char-count {@code LENGTH} and {@code position} / {@code strpos}) own those identifiers.
 */
@SuppressWarnings("unused")
public final class MySQLStringFunctionOperatorTable extends ReflectiveSqlOperatorTable {
    ...
    public MySQLStringFunctionOperatorTable() {
        init();
    }
}
