...
class ShardingSphereResultSetMetaDataTest {
    ...
    void assertReadingAllColumnHeadersReconcilesReturnedMetadataOnce() throws SQLException {
        ...
        assertTrue(sqlStatementContext.containsDerivedProjections());
        // The multi-route rewrite appended the derived order-by column and the backend schema drifted at the same time,
        // so every accessor reads through the reconciliation path, which must run once per metadata instance:
        // proxy query-header and MySQL prepare metadata generation read a whole header row through this class.
        ResultSetMetaData resultSetMetaData = mock(ResultSetMetaData.class);
        ...
        // The budget: the reconciliation decision and the visible-index mapping read the returned metadata once each,
        // then every accessor reads exactly its own column. Per-accessor reconciliation exceeds this budget.
        assertThat(countReadCount.get(), is(4));
        assertThat(labelReadCount.get(), is(2 * backendColumnLabels.length + 5));
    }
