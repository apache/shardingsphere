 ...
 * @author zhangliang 
 */
// TODO use SQLCasesLoader after all test cases can parse
public final class VisitorSQLCasesLoader {
    
    private final Map<String, SQLCase> sqlCases;
    
    public VisitorSQLCasesLoader(final String rootDirection) {
        sqlCases = loadSQLCases(rootDirection);
    }
    
    @SneakyThrows
    private static Map<String, SQLCase> loadSQLCases(final String path) {
        File file = new File(VisitorSQLCasesLoader.class.getProtectionDomain().getCodeSource().getLocation().getPath());
        ...
        try (JarFile jar = new JarFile(file)) {
            ...
            while (entries.hasMoreElements()) {
                ...
                if (name.startsWith(path + "/") && name.endsWith(".xml")) {
                    fillSQLMap(result, VisitorSQLCasesLoader.class.getClassLoader().getResourceAsStream(name));
                }
            }
        }
        return result;
    ...
    private static Map<String, SQLCase> loadSQLCasesFromTargetDirectory(final String path) {
        Map<String, SQLCase> result = new TreeMap<>();
        URL url = VisitorSQLCasesLoader.class.getClassLoader().getResource(path);
        if (null == url) {
            return result;
        }
    ...
    public String getSQL(final String sqlCaseId, final SQLCaseType sqlCaseType, final List<?> parameters) {
        String sql = getSQLFromMap(sqlCaseId, sqlCases);
        if (null == sql) {
            return null;
        }
        switch (sqlCaseType) {
            case Literal:
                return getLiteralSQL(sql, parameters);
            case Placeholder:
