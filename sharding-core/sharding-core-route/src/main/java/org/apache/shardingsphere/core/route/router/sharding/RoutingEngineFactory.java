...
public final class RoutingEngineFactory {
    ...
    public static RoutingEngine newInstance(final ShardingRule shardingRule, final ShardingDataSourceMetaData shardingDataSourceMetaData, final OptimizedStatement optimizedStatement) {
        ...
        if (optimizedStatement instanceof DMLOptimizedStatement && ((DMLOptimizedStatement) optimizedStatement).getShardingConditions().isAlwaysFalse() || tableNames.isEmpty()) {
            return new UnicastRoutingEngine(shardingRule, tableNames);
        }
        Preconditions.checkState(optimizedStatement instanceof DMLOptimizedStatement);
        return getShardingRoutingEngine(shardingRule, (DMLOptimizedStatement) optimizedStatement, tableNames);
    }
    ...
    private static RoutingEngine getShardingRoutingEngine(final ShardingRule shardingRule, final DMLOptimizedStatement optimizedStatement, final Collection<String> tableNames) {
