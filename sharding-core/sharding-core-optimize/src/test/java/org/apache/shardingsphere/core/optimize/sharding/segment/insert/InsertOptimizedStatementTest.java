...
public final class InsertOptimizedStatementTest {
    ...
    public void assertAddUnitWithSet() {
        ...
        InsertOptimizeResultUnit unit = insertClauseOptimizedStatement.createUnit("id", Collections.<String>emptyList(), expressions, parameters, 1);
        insertClauseOptimizedStatement.addUnit(unit);
        assertThat(insertClauseOptimizedStatement.getUnits().get(0).getValueExpressions(), is(expressions));
        ...
        assertThat(insertClauseOptimizedStatement.getUnits().get(0).getValue("value"), is((Object) "parameter"));
        assertThat(insertClauseOptimizedStatement.getUnits().get(0).getValue("status"), is((Object) "test"));
        insertClauseOptimizedStatement.getUnits().get(0).setValue("id", 2);
        assertThat(insertClauseOptimizedStatement.getUnits().get(0).getValue("id"), is((Object) 2));
        insertClauseOptimizedStatement.getUnits().get(0).setValue("value", "parameter1");
        assertThat(insertClauseOptimizedStatement.getUnits().get(0).getValue("value"), is((Object) "parameter1"));
    }
}
