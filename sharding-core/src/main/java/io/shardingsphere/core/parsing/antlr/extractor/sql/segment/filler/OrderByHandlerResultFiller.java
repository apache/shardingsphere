...
/**
 * Order by handler result filler.
 * 
 * @author duhongjun
 */
public class OrderByHandlerResultFiller extends AbstractHandlerResultFiller {
    
    public OrderByHandlerResultFiller() {
        super(OrderByExtractResult.class);
    }
    ...
    protected void fillSQLStatement(final Object extractResult, final SQLStatement statement, final ShardingRule shardingRule, final ShardingTableMetaData shardingTableMetaData) {
        OrderByExtractResult orderExtractResult = (OrderByExtractResult) extractResult;
        SelectStatement selectStatement = (SelectStatement) statement;
        if(-1 < orderExtractResult.getIndex()) {
            selectStatement.getOrderByItems().add(new OrderItem(orderExtractResult.getIndex(), orderExtractResult.getOrderDirection(), orderExtractResult.getNullOrderDirection())); 
            ...
            selectStatement.getOrderByItems().add(new OrderItem(owner, name, orderExtractResult.getOrderDirection(), orderExtractResult.getNullOrderDirection(), Optional.<String>absent())); 
        }
    }
}
