...
import io.shardingsphere.core.parsing.antlr.extractor.statement.handler.result.ColumnExtractResult;
...
public final class ConditionExtractHandler implements ASTExtractHandler<Optional<ConditionExtractResult>> {
    ...
    private Optional<OrCondition> extractConditionForParen(final Map<ParserRuleContext, Integer> questionNodeIndexMap, final ParserRuleContext exprNode) {
        int index = -1;
        for (int i = 0; i < exprNode.getChildCount(); i++) {
            if (OperatorUtils.isStartParen(exprNode.getChild(i).getText())) {
                index = i;
                break;
            }
        }
        if (0 <= index) {
            if (exprNode.getChildCount() != index + 3) {
                throw new RuntimeException("invalid expression");
            }
            if (OperatorUtils.parenMatch(exprNode.getChild(index + 2).getText(), exprNode.getChild(index).getText())) {
    ...
    private Optional<Condition> buildEqualCondition(final Map<ParserRuleContext, Integer> questionNodeIndexMap, final ParserRuleContext exprNode) {
        Optional<ParserRuleContext> comparisionNode = ASTUtils.findFirstChildNode(exprNode, RuleName.COMPARSIONOPERATOR);
    ...
    private Optional<Condition> buildInCondition(final Map<ParserRuleContext, Integer> questionNodeIndexMap, final ParserRuleContext predicateNode) {
        ...
        Collection<ParserRuleContext> expressionNodes = ASTUtils.getAllDescendantNodes((ParserRuleContext)predicateNode.getChild(0), RuleName.SIMPLEEXPR);
