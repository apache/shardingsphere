 ...
 * @author duhongjun
 */
public class SelectExpressionExtractor implements CollectionSQLSegmentExtractor<SelectExpressionExtractResult> {

    @Override
    public Collection<SelectExpressionExtractResult> extract(ParserRuleContext ancestorNode) {
        ...
        Collection<SelectExpressionExtractResult> result = new LinkedList<>();
        for(ParserRuleContext each : selectExprNodes) {
            ...
            if(firstChildText.endsWith(Symbol.STAR.getLiterals())) {
                ...
                result.add(new StarSelectExpressionExtractResult(Optional.of(owner)));
            }else {
                ...
                result.add(new CommonSelectExpressionExtractResult(builder.toString(), Optional.of(alias)));
            }
        }
        return Collections.emptyList();
    }
