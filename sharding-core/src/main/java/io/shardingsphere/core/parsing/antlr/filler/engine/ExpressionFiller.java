 ...
 * @author duhongjun
 */
public class ExpressionFiller implements SQLSegmentFiller {
    ...
    public void fill(SQLSegment sqlSegment, SQLStatement sqlStatement, ShardingRule shardingRule, ShardingTableMetaData shardingTableMetaData) {
        ...
        if (sqlSegment instanceof PropertyExpressionSegment) {
            fillPropertyExpression((PropertyExpressionSegment) sqlSegment, selectStatement);
            return;
        }
        if (sqlSegment instanceof CommonExpressionSegment) {
            CommonExpressionSegment commonSegment = (CommonExpressionSegment) sqlSegment;
            String expression = selectStatement.getSql().substring(commonSegment.getStartPosition(), commonSegment.getEndPosition() + 1);
        ...
        if (sqlSegment instanceof FunctionExpressionSegment) {
            fillFunctionExpression((FunctionExpressionSegment) sqlSegment, selectStatement);
            return;
        }
    ...
    private void fillPropertyExpression(PropertyExpressionSegment propertySegment, SelectStatement selectStatement) {
        ...
        String expression = selectStatement.getSql().substring(propertySegment.getStartPosition(), propertySegment.getEndPosition() + 1);
        selectStatement.getItems().add(new CommonSelectItem(expression,propertySegment.getAlias()));
    }   
    
    private void fillFunctionExpression(FunctionExpressionSegment functionSegment, SelectStatement selectStatement) {
