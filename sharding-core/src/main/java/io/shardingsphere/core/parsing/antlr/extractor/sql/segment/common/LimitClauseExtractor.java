 ...
 * @author duhongjun
 */
public class LimitClauseExtractor implements CollectionSQLSegmentExtractor<LimitExtractResult> {
    
    @Override
    public Collection<LimitExtractResult> extract(ParserRuleContext ancestorNode) {
        ...
        Collection<LimitExtractResult> result = new LinkedList<>();
    ...
    public void addLimitExtractResult(Collection<LimitExtractResult> limitResult, Map<ParserRuleContext, Integer> questionNodeIndexMap, ParseTree node) {
        if(node.getText().equals(Symbol.QUESTION.getLiterals())) {
            if(questionNodeIndexMap.containsKey(node)) {
                limitResult.add(new LimitExtractResult(DatabaseType.MySQL, -1 , questionNodeIndexMap.get(node)));
            }
        }else {
            limitResult.add(new LimitExtractResult(DatabaseType.MySQL, NumberUtil.getExactlyNumber(node.getText(), 10).intValue() , -1));
        }
    }
}
