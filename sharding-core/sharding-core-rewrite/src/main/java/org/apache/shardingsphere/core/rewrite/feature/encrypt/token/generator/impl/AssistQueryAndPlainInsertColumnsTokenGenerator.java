...
public final class AssistQueryAndPlainInsertColumnsTokenGenerator implements OptionalSQLTokenGenerator, EncryptRuleAware {
    ...
    public AssistQueryAndPlainInsertColumnsToken generateSQLToken(final SQLStatementContext sqlStatementContext) {
        ...
        return new AssistQueryAndPlainInsertColumnsToken(sqlSegment.get().getStopIndex(), getEncryptDerivedColumnNames(sqlSegment.get(), tableName));
    }
    
    private List<String> getEncryptDerivedColumnNames(final InsertColumnsSegment sqlSegment, final String tableName) {
