...
public final class SelectItemsTokenGenerator implements OptionalSQLTokenGenerator<ShardingRule>, IgnoreForSingleRoute {
    ...
    public Optional<SelectItemsToken> generateSQLToken(
        ...
        return derivedItemTexts.isEmpty() ? Optional.<SelectItemsToken>absent()
                : Optional.of(
                        new SelectItemsToken(((SelectSQLStatementContext) rewriteStatement.getSqlStatementContext()).getProjectionsContext().getStopIndex() + 1 + " ".length(), derivedItemTexts));
    }
    
    private Collection<String> getDerivedItemTexts(final SelectSQLStatementContext selectSQLStatementContext) {
        Collection<String> result = new LinkedList<>();
        for (SelectItem each : selectSQLStatementContext.getProjectionsContext().getItems()) {
