...
public final class ShardingSQLRewriteEngineTest {
    ...
    private SQLRouteResult createRouteResultForAutoGeneratedKeyColumn() {
        ...
        ShardingInsertOptimizedStatement shardingStatement = new ShardingInsertOptimizedStatement(insertStatement, new LinkedList<ShardingCondition>(), Arrays.asList("name", "age"), null);
        ...
        shardingStatement.getInsertValues().add(new InsertValue(assignments, 0, Arrays.<Object>asList("x", 1, 1), 0));
    ...
    private SQLRouteResult createRouteResultForAutoGeneratedKeyColumnWithoutColumnsWithParameter() {
        ...
        shardingStatement.getInsertValues().add(new InsertValue(assignments, 0, Arrays.<Object>asList("Bill", 1), 0));
        ShardingCondition shardingCondition = new ShardingCondition();
        shardingCondition.getDataNodes().add(new DataNode("db0.table_1"));
        shardingStatement.getShardingConditions().getConditions().add(shardingCondition);
        SQLRouteResult result = new SQLRouteResult(shardingStatement, new EncryptTransparentOptimizedStatement(insertStatement));
