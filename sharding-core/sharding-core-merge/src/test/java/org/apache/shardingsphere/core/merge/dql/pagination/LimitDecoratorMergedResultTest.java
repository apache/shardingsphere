...
public final class LimitDecoratorMergedResultTest {
    ...
    public void setUp() throws SQLException {
        ...
        SelectOptimizedStatement optimizedStatement = new SelectOptimizedStatement(new SelectStatement(), Collections.<ShardingCondition>emptyList());
    ...
    public void assertNextForSkipAll() throws SQLException {
        ((SelectOptimizedStatement) routeResult.getOptimizedStatement()).setPagination(new Pagination(new NumberLiteralLimitValueSegment(0, 0, Integer.MAX_VALUE), null, Collections.emptyList()));
    ...
    public void assertNextWithoutRowCount() throws SQLException {
        ((SelectOptimizedStatement) routeResult.getOptimizedStatement()).setPagination(new Pagination(new NumberLiteralLimitValueSegment(0, 0, 2), null, Collections.emptyList()));
        mergeEngine = new DQLMergeEngine(DatabaseTypes.getActualDatabaseType("MySQL"), routeResult, queryResults);
        MergedResult actual = mergeEngine.merge();
        for (int i = 0; i < 6; i++) {
