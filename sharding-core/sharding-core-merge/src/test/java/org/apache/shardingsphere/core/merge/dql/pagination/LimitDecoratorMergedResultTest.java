...
public final class LimitDecoratorMergedResultTest {
    ...
    public void setUp() throws SQLException {
        ...
        routeResult.setPagination(new Pagination(null, null, Collections.emptyList()));
    }
    
    @Test
    public void assertNextForSkipAll() throws SQLException {
        routeResult.setPagination(new Pagination(new NumberLiteralLimitValueSegment(0, 0, Integer.MAX_VALUE), null, Collections.emptyList()));
    ...
    public void assertNextWithoutRowCount() throws SQLException {
        routeResult.setPagination(new Pagination(new NumberLiteralLimitValueSegment(0, 0, 2), null, Collections.emptyList()));
        mergeEngine = new DQLMergeEngine(DatabaseType.MySQL, routeResult, queryResults);
        MergedResult actual = mergeEngine.merge();
        for (int i = 0; i < 6; i++) {
    ...
    public void assertNextWithRowCount() throws SQLException {
        routeResult.setPagination(new Pagination(new NumberLiteralLimitValueSegment(0, 0, 2), new NumberLiteralLimitValueSegment(0, 0, 2), Collections.emptyList()));
        mergeEngine = new DQLMergeEngine(DatabaseType.MySQL, routeResult, queryResults);
        MergedResult actual = mergeEngine.merge();
        assertTrue(actual.next());
        assertTrue(actual.next());
