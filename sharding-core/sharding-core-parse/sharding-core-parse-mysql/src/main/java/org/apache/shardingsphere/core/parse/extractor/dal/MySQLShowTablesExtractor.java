...
public final class MySQLShowTablesExtractor implements OptionalSQLSegmentExtractor {
    ...
    public Optional<ShowTablesSegment> extract(final ParserRuleContext ancestorNode, final Map<ParserRuleContext, Integer> parameterMarkerIndexes) {
        Optional<ParserRuleContext> fromSchema = ExtractorUtils.findFirstChildNodeNoneRecursive(ancestorNode, RuleName.FROM_SCHEMA);
        return Optional.of(fromSchema.isPresent() ? new ShowTablesSegment(fromSchema.get().getStart().getStartIndex(), fromSchema.get().getStop().getStopIndex()) : new ShowTablesSegment());
    }
}
