...
public final class PredicateExtractor implements OptionalSQLSegmentExtractor {
    ...
    private Optional<PredicateSegment> extractComparisonPredicate(final ParserRuleContext exprNode, final Map<ParserRuleContext, Integer> parameterMarkerIndexes) {
        Optional<ParserRuleContext> booleanPrimaryNode = ExtractorUtils.findSingleNodeFromFirstDescendant(exprNode, RuleName.BOOLEAN_PRIMARY);
        ...
        Optional<ParserRuleContext> leftColumnNode = ExtractorUtils.findSingleNodeFromFirstDescendant((ParserRuleContext) booleanPrimaryNode.get().getChild(0), RuleName.COLUMN_NAME);
        Optional<ParserRuleContext> rightColumnNode = ExtractorUtils.findSingleNodeFromFirstDescendant((ParserRuleContext) booleanPrimaryNode.get().getChild(2), RuleName.COLUMN_NAME);
