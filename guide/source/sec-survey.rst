...
Структура строковых выражений
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Row Expression состоит из двух частей, объединённых в одну строку:

- Тип выражения, соответствующий реализации SPI, указывается в угловых скобках ``< >``.

- Само выражение, описывающее диапазон или набор значений.
...
Здесь ``<GROOVY>`` — это тип, указывающий реализацию SPI для обработки выражения.

Строка ``t_order_${1..3}`` — это само выражение, определяющее последовательность значений.

Если в тип выражения не указан (например, ``t_order_${1..3}``), то по умолчанию 
используется реализация ``GROOVY`` — стандартный парсер ``InlineExpressionParser`` SPI.

Поддерживаемые выражения и синтаксис
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Строковые выражения используют синтаксис ``Groovy``, что позволяет применять все стандартные операции ``Groovy`` внутри выражений.
В конфигурации допустимо использовать шаблоны вида: ``${ expression }`` или ``$->{ expression }``.
...
3. *Комбинированное выражение*
   ...
   Если в одном выражении строки присутствует несколько шаблонов ``${ expression }`` или ``$->{ expression }``, 
