...
/**
 * Optimize logic schema generator.
 */
public final class OptimizeLogicSchemaGenerator {
    
    private final String name;
    
    private final Map<String, OptimizeLogicTableGenerator> tables = new LinkedMap<>();
    
    public OptimizeLogicSchemaGenerator(final ShardingSphereMetaData metaData) throws SQLException {
    ...
    private void initTables(final ShardingSphereMetaData metaData) throws SQLException {
        ...
        for (Map.Entry<String, Collection<DataNode>> entry : tableDataNodes.entrySet()) {
            tables.put(entry.getKey(),
                    new OptimizeLogicTableGenerator(entry.getKey(), metaData.getResource().getDataSources(), dataSourceRules, entry.getValue(), metaData.getResource().getDatabaseType()));
        }
    }
     ...
     * @return schema
     */
    public OptimizeLogicSchema create(final OptimizeRowExecutor executor) {
        Map<String, Table> tables = new LinkedMap<>();
        for (Entry<String, OptimizeLogicTableGenerator> entry : this.tables.entrySet()) {
            tables.put(entry.getKey(), entry.getValue().create(executor));
        }
        return new OptimizeLogicSchema(name, tables);
    }
}
