...
public final class ColumnConverter implements SQLSegmentConverter<ColumnSegment, SqlNode> {
    ...
    public Optional<ColumnSegment> convertToSQLSegment(final SqlNode sqlNode) {
        ...
        if (sqlNode instanceof SqlIdentifier) {
            ...
            if (1 == names.size()) {
                return Optional.of(new ColumnSegment(getStartIndex(sqlNode), getStopIndex(sqlNode), new IdentifierValue(names.get(0))));
            }
            ColumnSegment result = new ColumnSegment(getStartIndex(sqlNode), getStopIndex(sqlNode), new IdentifierValue(names.get(1)));
            SqlIdentifier owner = ((SqlIdentifier) sqlNode).getComponent(0);
