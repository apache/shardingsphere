 ...
 */
@Getter
public final class OracleMultiInsertStatementContext extends CommonSQLStatementContext<InsertStatement> implements TableAvailable, ParameterAware {
    ...
    private final Map<Integer, List<String>> insertColumnNames = new LinkedHashMap<>();
    ...
    private Map<Integer, GeneratedKeyContext> generatedKeyContext = new LinkedHashMap<>();
    
    private Integer cursor = 0;
    
    private Collection<InsertStatement> insertStatements;
    
    public OracleMultiInsertStatementContext(final Map<String, ShardingSphereMetaData> metaDataMap, final List<Object> parameters,
                                             final OracleInsertStatement oracleInsertStatement, final String defaultDatabaseName) {
        super(oracleInsertStatement);
        ...
        oracleInsertStatement.getInsertMultiTableElementSegment().ifPresent(each -> {
            insertStatements = each.getInsertStatements();
            for (InsertStatement insertStatement : insertStatements) {
                List<String> insertColumnName = getInsertColumnNames(insertStatement);
                insertColumnNames.put(cursor, insertColumnName);
                ...
                insertValueContexts.put(cursor, getInsertValueContexts(parameters, parametersOffset, valueExpression));
                ...
                columnNames.put(cursor, columnName);
                generatedKeyContext.put(cursor, new GeneratedKeyContextEngine(insertStatement, schema)
                        .createGenerateKeyContext(insertColumnName, getAllValueExpressions(insertStatement), parameters).orElse(null));
                cursor++;
            }
        });
     ...
     */
    public List<String> getInsertColumnNames(final InsertStatement insertStatement) {
        Optional<SetAssignmentSegment> setAssignment = InsertStatementHandler.getSetAssignmentSegment(insertStatement);
    ...
    public void setUpParameters(final List<Object> parameters) {
        ...
        Integer cursor = 0;
        for (InsertStatement insertStatement : insertStatements) {
            List<List<ExpressionSegment>> valueExpression = valueExpressions.get(cursor);
            insertValueContexts.put(cursor, getInsertValueContexts(parameters, parametersOffset, valueExpression));
            parametersOffset.getAndAdd(valueExpression.size());
            generatedKeyContext.put(cursor, new GeneratedKeyContextEngine(insertStatement, schema).createGenerateKeyContext(insertColumnNames.get(cursor), valueExpression, parameters).orElse(null));
            cursor++;
        }
        insertSelectContext = getInsertSelectContext(metaDataMap, parameters, parametersOffset, defaultDatabaseName).orElse(null);
        onDuplicateKeyUpdateValueContext = getOnDuplicateKeyUpdateValueContext(parameters, parametersOffset).orElse(null);
