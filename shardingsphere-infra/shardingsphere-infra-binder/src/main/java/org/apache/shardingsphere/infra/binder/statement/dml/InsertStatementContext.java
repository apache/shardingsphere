...
public final class InsertStatementContext extends CommonSQLStatementContext<InsertStatement> implements TableAvailable {
    ...
    public InsertStatementContext(final List<Object> parameters, final InsertStatement sqlStatement) {
        super(sqlStatement);
        AtomicInteger parametersOffset = new AtomicInteger(0);
        insertValueContexts = getInsertValueContexts(parameters, parametersOffset);
        insertSelectContext = getInsertSelectContext(parameters, parametersOffset).orElse(null);
    ...
    public void initSchemaBasedContext(final ShardingSphereSchema schema) {
        ...
        if (null != insertSelectContext) {
            insertSelectContext.getSelectStatementContext().initSchemaBasedContext(schema);
        }
    ...
    private Optional<InsertSelectContext> getInsertSelectContext(final List<Object> parameters, final AtomicInteger parametersOffset) {
        if (!getSqlStatement().getInsertSelect().isPresent()) {
            return Optional.empty();
        }
        SubquerySegment insertSelectSegment = getSqlStatement().getInsertSelect().get();
        SelectStatementContext selectStatementContext = new SelectStatementContext(parameters, insertSelectSegment.getSelect());
