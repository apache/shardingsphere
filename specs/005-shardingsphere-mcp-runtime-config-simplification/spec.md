...
- 保留 `schemaPattern` 与 `defaultSchema`，但把它们明确为 metadata 范围配置
...
2. **Given** `runtime.databaseDefaults` 与某个 database binding 同时声明 metadata
   范围，**When** 加载配置，**Then** per-database 配置优先，shared defaults 只补缺省值。
...
- `runtime.databaseDefaults.metadata` 与
  `runtime.databases.<name>.metadata` 同时声明时，per-database metadata 优先。
...
- `schemaPattern` 与 `defaultSchema` 仍属于 deployment-specific metadata scope，
  不应被误删成“数据库固有能力”。
...
- **FR-008**: 每个 logical database binding MAY 显式声明 `username`、
  `password`、`driverClassName` 和 `metadata` 子配置。
...
- **FR-011**: `schemaPattern` 与 `defaultSchema` MUST 继续可配置，
  因为它们描述 metadata 范围和 fallback 行为，而不是数据库固有 capability。
- **FR-012**: `schemaPattern` 与 `defaultSchema` SHOULD 位于 direct runtime
  binding 的 `metadata` 子配置下，并支持从 `runtime.databaseDefaults.metadata` 继承。
...
- **Logical Database Binding Configuration**: 一个 logical database 的 direct
  JDBC binding，包含连接输入与 metadata scope 输入。
- **Metadata Scope Configuration**: `schemaPattern`、`defaultSchema` 等决定
  metadata 装载边界与 fallback 行为的配置对象。
