...
- 使用 `test/e2e/mcp` 里的 production bootstrap runtime harness
- 同时覆盖 file-backed H2 runtime 和自动拉起的 MySQL Docker runtime
- 使用本地自托管小模型服务
...
**Primary Dependencies**: existing `test/e2e/mcp` module, MCP bootstrap runtime harness, local HTTP-accessible Ollama model service, Jackson/Java `HttpClient`, MySQL JDBC driver, Testcontainers MySQL support  
**Storage**: file-backed H2 seed data plus seeded MySQL Docker data, per-run artifacts under `test/e2e/mcp/target/llm-e2e/<run-id>/`  
...
**Constraints**: no branch switch; no external SaaS model secret in default path; read-only only; no replacement of deterministic E2E; artifact directories must be isolated per run  
**Scale/Scope**: two minimal smoke scenarios, one default model profile (`Ollama + qwen3:1.7b`), one dedicated workflow, one local quickstart path
...
test/e2e/mcp/src/test/java/org/apache/shardingsphere/test/e2e/mcp/LLME2EConfiguration.java
test/e2e/mcp/src/test/java/org/apache/shardingsphere/test/e2e/mcp/LLME2EArtifactBundle.java
test/e2e/mcp/src/test/java/org/apache/shardingsphere/test/e2e/mcp/LLMStructuredAnswer.java
test/e2e/mcp/src/test/java/org/apache/shardingsphere/test/e2e/mcp/MCPToolTraceRecord.java
test/e2e/mcp/src/test/java/org/apache/shardingsphere/test/e2e/mcp/LLMChatModelClient.java
test/e2e/mcp/src/test/java/org/apache/shardingsphere/test/e2e/mcp/LLMMCPConversationRunner.java
test/e2e/mcp/src/test/java/org/apache/shardingsphere/test/e2e/mcp/AbstractLLMMCPE2ETest.java
test/e2e/mcp/src/test/java/org/apache/shardingsphere/test/e2e/mcp/ProductionLLMH2SmokeE2ETest.java
test/e2e/mcp/src/test/java/org/apache/shardingsphere/test/e2e/mcp/ProductionLLMMySQLSmokeE2ETest.java
test/e2e/mcp/src/test/java/org/apache/shardingsphere/test/e2e/mcp/*Test.java
...
test/e2e/mcp/src/test/java/org/apache/shardingsphere/test/e2e/mcp/MySQLRuntimeTestSupport.java
test/e2e/mcp/src/test/java/org/apache/shardingsphere/test/e2e/mcp/H2RuntimeTestSupport.java
mcp/README.md
mcp/README_ZH.md
```

**Structure Decision**: 不新增新的 top-level Maven module；
优先把 LLM runner 直接放在 `test/e2e/mcp` 现有包下，
这样能最大化复用现有 MCP E2E harness、package-private helper 和 GitHub Actions 构建上下文。
...
### 2. 第一阶段验收固定为 H2 + MySQL 双场景

- H2 使用 `H2RuntimeTestSupport` 创建 file-backed JDBC runtime
- MySQL 通过 Docker / Testcontainers 自动拉起并注入等价种子数据
- 两条 smoke 都验证 `orders` 表总数为 `2`
- 不再把 MySQL 延后到 follow-up
...
### 4. 模型服务接入采用可配置的本地 HTTP chat profile，但默认固定为 Ollama

- runner 通过配置接入本地模型服务
- 默认 smoke 固定为 `Ollama + qwen3:1.7b`
...
### 6. workflow 先独立，以 workflow_dispatch + nightly 交付
...
1. `model_must_use_real_mcp_tool_calls`
   Planned verification: `ProductionLLMH2SmokeE2ETest` 与 `ProductionLLMMySQLSmokeE2ETest` 断言 trace 同时包含 discovery tool 与 `execute_query`
2. `smoke_contract_must_stay_read_only_and_deterministic`
   Planned verification: runner 拒绝非只读 SQL，最终 JSON 断言 H2 与 MySQL 两条场景的 `logic_db -> orders -> total_orders=2`
3. `failure_diagnostics_must_be_complete`
   Planned verification: artifact bundle 包含 prompt、raw output、tool trace、assertion report、MCP interaction log
...
3. 基于现有 production runtime harness 增加 H2 与 MySQL 的独立 smoke tests。
...
```bash
./mvnw -pl test/e2e/mcp -am -DskipITs -Dspotless.skip=true \
  -Dtest=LLME2EConfigurationTest,LLMChatModelClientTest,LLMMCPConversationRunnerTest,LLME2EArtifactWriterTest,LLMStructuredAnswerTest,MCPHttpToolClientTest test \
  -Dsurefire.failIfNoSpecifiedTests=false
```
...
- 如果后续需要验证客户真实数据库拓扑，应另开 follow-up spec，
  在 H2 + MySQL 之上扩展新的 runtime profile，而不是污染当前最小 contract。
