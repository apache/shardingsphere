...
//    private static ShardingType shardingType = ShardingType.SHARDING_READ_WRITE_SPLITTING;
    ...
    private static String getApplicationFile() {
        switch (shardingType) {
            ...
            case SHARDING_READ_WRITE_SPLITTING:
                return String.format("META-INF/%s/%s/application-read-write-splitting.xml", registryCenterType.name().toLowerCase(), loadConfigFromRegCenter ? "cloud" : "local");
            case ENCRYPT:
