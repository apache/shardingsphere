...
import org.apache.shardingsphere.mode.node.path.reservation.WorkerIDReservationNodePath;
    ...
    public Optional<Integer> reserveWorkerId(final Integer preselectedWorkerId, final String instanceId) {
        try {
            return repository.persistExclusiveEphemeral(
                    NewNodePathGenerator.generatePath(new WorkerIDReservationNodePath(preselectedWorkerId), false), instanceId) ? Optional.of(preselectedWorkerId) : Optional.empty();
