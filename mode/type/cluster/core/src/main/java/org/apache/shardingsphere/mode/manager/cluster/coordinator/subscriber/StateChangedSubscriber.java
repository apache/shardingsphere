...
public final class StateChangedSubscriber {
    ...
    public synchronized void renew(final PrimaryStateChangedEvent event) {
        ...
        QualifiedDatabase qualifiedDatabase = event.getQualifiedDatabase();
        for (DynamicDataSourceContainedRule each : contextManager
                .getMetaDataContexts().getMetaData().getDatabase(qualifiedDatabase.getDatabaseName()).getRuleMetaData().findRules(DynamicDataSourceContainedRule.class)) {
