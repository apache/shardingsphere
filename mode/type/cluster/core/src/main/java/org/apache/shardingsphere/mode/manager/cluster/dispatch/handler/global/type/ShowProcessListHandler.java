...
public final class ShowProcessListHandler implements GlobalDataChangedEventHandler {
    ...
    public String getSubscribedKey() {
        return NodePathGenerator.toPath(new ShowProcessListTriggerNodePath(null, null), false);
    }
    ...
    private Matcher getShowProcessListTriggerMatcher(final DataChangedEvent event) {
        return Pattern.compile(NodePathGenerator.toPath(new ShowProcessListTriggerNodePath(null, null), false) + "/([\\S]+):([\\S]+)$", Pattern.CASE_INSENSITIVE).matcher(event.getKey());
    }
}
