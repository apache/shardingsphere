...
class ClusterProcessPersistServiceTest {
    ...
    private void assertGetProcessList() {
        when(repository.getChildrenKeys(ComputeNodePathGenerator.getOnlinePath(InstanceType.JDBC))).thenReturn(Collections.emptyList());
        when(repository.getChildrenKeys(ComputeNodePathGenerator.getOnlinePath(InstanceType.PROXY))).thenReturn(Collections.singletonList("abc"));
