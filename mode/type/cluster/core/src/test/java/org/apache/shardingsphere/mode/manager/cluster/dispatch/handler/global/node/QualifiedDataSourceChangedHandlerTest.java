...
class QualifiedDataSourceChangedHandlerTest {
    ...
    void setUp() {
        handler = ShardingSphereServiceLoader.getServiceInstances(GlobalDataChangedEventHandler.class).stream()
                .filter(each -> NodePathGenerator.toPath(each.getSubscribedNodePath(), false).equals("/nodes/qualified_data_sources")).findFirst().orElse(null);
    }
    ...
    void assertHandleWithEmptyValue() {
        ...
        verify(contextManager.getMetaDataContexts().getMetaData(), times(0)).containsDatabase(any());
    }
