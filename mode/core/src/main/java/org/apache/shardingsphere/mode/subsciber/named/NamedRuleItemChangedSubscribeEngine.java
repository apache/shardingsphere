...
public final class NamedRuleItemChangedSubscribeEngine<T extends RuleConfiguration> {
     ...
     */
    public void renew(final AlterNamedRuleItemEvent event, final NamedRuleItemAlteredSubscribeCallback<T> callback) {
     ...
     */
    public void renew(final DropNamedRuleItemEvent event, final NamedRuleItemDroppedSubscribeCallback<T> callback) {
        if (!contextManager.getMetaDataContexts().getMetaData().containsDatabase(event.getDatabaseName())) {
            return;
        }
        ShardingSphereDatabase database = contextManager.getMetaDataContexts().getMetaData().getDatabases().get(event.getDatabaseName());
        T toBeDroppedConfig = callback.getToBeDroppedConfiguration(event.getItemName(), database);
