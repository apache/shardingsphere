...
public final class DataSourceUnitPersistService {
    ...
    public Map<String, DataSourcePoolProperties> load(final String databaseName) {
        ...
        return childrenKeys.stream().collect(Collectors.toMap(each -> each, each -> load(databaseName, each), (oldValue, currentValue) -> currentValue, () -> new LinkedHashMap<>(childrenKeys.size(), 1F)));
    }
