...
public final class UpdateStatementContext extends CommonSQLStatementContext implements TableSegmentsAvailable {
    ...
    public Collection<TableSegment> getAllTables() {
        UpdateStatement updateStatement = (UpdateStatement) getSqlStatement();
        Collection<TableSegment> result = new LinkedList<>(updateStatement.getTables());
        if (updateStatement.getWhere().isPresent()) {
            result.addAll(getAllTablesFromWhere(updateStatement.getWhere().get(), updateStatement.getTables()));
        }
        return result;
    }
