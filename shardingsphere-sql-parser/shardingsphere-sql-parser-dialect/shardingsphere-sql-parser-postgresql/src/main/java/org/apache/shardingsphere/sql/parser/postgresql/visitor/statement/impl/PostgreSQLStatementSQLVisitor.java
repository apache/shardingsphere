...
public abstract class PostgreSQLStatementSQLVisitor extends PostgreSQLStatementBaseVisitor<ASTNode> {
    ...
    public ASTNode visitInsertTarget(final InsertTargetContext ctx) {
        ...
        SimpleTableSegment result = new SimpleTableSegment(tableName.getStartIndex(), tableName.getStopIndex(), tableName);
        result.setOwner(owner);
        if (null != ctx.AS()) {
            ColIdContext colId = ctx.colId();
    ...
    public ASTNode visitRelationExprOptAlias(final RelationExprOptAliasContext ctx) {
        ...
        if (null != ctx.colId()) {
            ...
            result = new SimpleTableSegment(colId.start.getStartIndex(), colId.stop.getStopIndex(), tableName);
    ...
    public ASTNode visitTableReference(final TableReferenceContext ctx) {
        ...
        return new SimpleTableSegment(ctx.start.getStartIndex(), ctx.stop.getStopIndex(), tableName);
    }
    ...
    private SimpleTableSegment generateTableFromRelationExpr(final RelationExprContext ctx) {
        ...
        if (null != qualifiedName.indirection()) {
            ...
            SimpleTableSegment table = new SimpleTableSegment(tableName.start.getStartIndex(), tableName.stop.getStopIndex(), 
                    new TableNameSegment(tableName.start.getStartIndex(), tableName.stop.getStopIndex(), new IdentifierValue(tableName.getText())));
        ...
        return new SimpleTableSegment(qualifiedName.colId().start.getStartIndex(), qualifiedName.colId().stop.getStopIndex(),
                new TableNameSegment(qualifiedName.colId().start.getStartIndex(), qualifiedName.colId().stop.getStopIndex(), new IdentifierValue(qualifiedName.colId().getText())));
    }
