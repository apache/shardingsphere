    ...
    public static List<ParameterMarkerExpressionSegment> extractParameterMarkerExpressionSegment(final Collection<ExpressionSegment> expressions) {
        ...
        for (ExpressionSegment each : expressions) {
            if (each instanceof ParameterMarkerExpressionSegment) {
                ...
                if (((BetweenExpression) each).getBetweenExpr() instanceof BetweenExpression) {
                    result.add((ParameterMarkerExpressionSegment) ((BetweenExpression) each).getBetweenExpr());
                }

                if (((BetweenExpression) each).getAndExpr() instanceof BetweenExpression) {
                    result.add((ParameterMarkerExpressionSegment) ((BetweenExpression) each).getAndExpr());
                }
            } else if (each instanceof InExpression) {
                result.addAll(((InExpression) each).getExpressionList().stream().filter(expression -> expression instanceof ParameterMarkerExpressionSegment).map(expression -> (ParameterMarkerExpressionSegment) expression).collect(Collectors.toList()););
            }
        }
        return result;
    }
