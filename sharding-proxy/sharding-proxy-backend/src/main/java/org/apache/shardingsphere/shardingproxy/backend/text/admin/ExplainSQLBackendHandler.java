...
/**
 * Explain SQL backend handler.
 *
 * @author Wen
 */
@RequiredArgsConstructor
public class ExplainSQLBackendHandler implements TextProtocolBackendHandler {
    ...
    public BackendResponse execute() {
        ...
        routeUnits = statementExecutorWrapper.route(sql, LogicSchemas.getInstance().getDatabaseType()).getRouteUnits().iterator();

        queryHeaders = new ArrayList<>(2);
        queryHeaders.add(new QueryHeader("", "", "data_source_name", "", 255, JDBCType.VARCHAR.getVendorTypeNumber(), 0));
        queryHeaders.add(new QueryHeader("", "", "sql", "", 255, JDBCType.VARCHAR.getVendorTypeNumber(), 0));
