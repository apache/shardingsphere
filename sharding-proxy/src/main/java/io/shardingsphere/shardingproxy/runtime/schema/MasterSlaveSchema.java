...
public final class MasterSlaveSchema extends LogicSchema {
    ...
    private ShardingMetaData createShardingMetaData() {
        setShardingRule(new ShardingRule(new ShardingRuleConfiguration(), getDataSources().keySet()));
        return new ShardingMetaData(getDataSourceURLs(getDataSources()), getShardingRule(),
