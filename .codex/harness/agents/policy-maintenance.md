...
5. A bare `run.py` invocation defaults to deterministic `--mode validate`; start networked semantic evaluation only with explicit `--mode semantic` or `--mode all`. For deterministic-only changes, run `--mode validate` and applicable source-read traces. For transport-only changes, also prove batch coverage, limits, concurrency, failure propagation, no-retry behavior, and aggregation with deterministic tests before the smallest live `--transport-check`.
