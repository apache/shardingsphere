...
4. Partition selected semantic cases under the input limit, run no more than two batches concurrently, evaluate each case exactly once, and never retry an unchanged failed batch.
5. Use `--semantic-stability-check` only with an original V0 from the same evaluator model and reasoning effort, the same case IDs, and complete single-pass failure metadata. It passes only when transport is exact, deterministic changes are authorized, the single-pass pass count is not lower, and no more-permissive decision, forbidden action, or unauthorized high-risk action is introduced. Wording and redundant action mismatches remain reported but are not independently blocking when this check passes.
6. Compare pass rate, duration, input tokens, and uncached input tokens only for identical case sets. Treat deterministic, transport, aggregate, or safety regressions as blocking and inspect bounded logs before another edit.
7. Only a candidate passing every check required by its impact classification may enter bootstrap, completion review, or handoff.
