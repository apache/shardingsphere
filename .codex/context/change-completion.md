...
The final response must lead with the outcome and include changed files and rationale, verification status, remaining risks, and the next action only when one is still required. Every code-writing handoff must explicitly report the completed code review result and review recommendations. Put a value on separate `Review Result:`, `Blocking Issues:`, `Required Changes:`, and `Review Recommendations:` lines so the project Stop hook can validate the handoff after `apply_patch` changes a non-prose file. State the blocking issue count and required changes, and explicitly state `No changes recommended` when no findings exist. Include a non-empty `Commit Message:` line when the blocking issue count is zero, and omit it when the task cannot be completed. Do not omit the review output merely because the review was performed internally. Validation results, passing tests, a bare `Review Result: Mergeable`, or commit artifacts do not substitute for review recommendations. Include command details and commit artifacts only when required by the applicable review path or requested by the user.
