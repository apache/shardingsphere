...
5. Invoke `$review-pr` in Formal Review Mode when the user requests a PR, mergeability, or formal review; the task targets or backs a PR; or the candidate changes a public API, SPI, externally visible contract, module dependency, security or concurrency behavior, or user-visible runtime performance, or contains deletion, an irreversible action, last-consumer removal, or unresolved evidence. An ordinary internal implementation or build change uses bounded self-review when step 4 is satisfied. For a PR-backed candidate, apply its public-head and remote-evidence checks; for a standalone triggered candidate, review only the original task baseline and task-owned delta.
