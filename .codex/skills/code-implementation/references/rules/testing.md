...
- Test SQL and DistSQL `StatementVisitor` behavior, including behavior reached through `SQLStatementVisitorEngine`, in Parser IT; do not create or update a unit test for that behavior.
- When an in-scope existing unit test covers this behavior, migrate every distinct accepted SQL and expected result, or rejected SQL, to Parser IT before removing the unit test. Extend Parser IT assertions only when needed to preserve an existing contract.
