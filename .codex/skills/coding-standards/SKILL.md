...
- Before the first write, read the portions of `CODE_OF_CONDUCT.md`, repository-level and path-level `AGENTS.md`, Checkstyle, Spotless, and other written repository standards that actually apply to every affected file, then record a compact applicable-rule checklist; group files governed by the same rules instead of repeating the checklist.
