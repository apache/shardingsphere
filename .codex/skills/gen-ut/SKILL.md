...
- `R1`：遵循 `CODE_OF_CONDUCT.md`。
- `R2`：使用 JUnit `@Test`；禁止 `@RepeatedTest`。
- `R3`：改动范围严格限制在由输入目标类解析出的测试范围内。
  - 允许编辑文件仅 `<ResolvedTestFileSet>`。
  - 禁止为修复无关构建/检查/门禁失败修改其他测试文件。
  - 仅当用户在当前轮次明确批准时可扩范围。
- `R4`：分支路径规则：编码前枚举目标公开方法全部分支路径；默认一个分支/路径仅映射一个测试方法；同分支新增测试必要性按 `R12` 判定。
- `R5`：每个测试覆盖一个场景，且对目标公开方法最多调用一次；同场景可附加断言。
- `R6`：被测类可通过 SPI 获取时，默认使用 `TypedSPILoader` 和 `OrderedSPILoader`。
  - “可通过 SPI 获取”：类实现 `TypedSPI`/`DatabaseTypedSPI`，或其实现可被 SPI 加载器发现。
  - 不使用 SPI 实例化时，必须在实现前于计划中记录原因。
- `R7`：目标类已有相关测试类时必须原位更新：先补缺失覆盖，再按 `R12` 删除或合并覆盖等价测试；仅在无相关测试类时新建。
- `R8`：存在死代码时，报告类名、文件路径、精确行号和不可达原因。
- `R9`：完成判定满足以下之一：
  - `R9-A`：目标类覆盖率（类/行/分支）100%，目标测试类 Surefire 成功执行，且必需质量门禁（`R11` 硬门禁、Checkstyle）通过，并附命令与退出码。
  - `R9-B`：若在“不改生产代码”规则下死代码阻塞 100% 分支覆盖率，按 `R8` 报告并标记阻塞。
  - `R9-C`：若失败发生在 `R3` 范围之外，按 `R10` 报告阻塞证据并标记阻塞。
  - 优先级：先判 `R9-B`，再判 `R9-C`；两者都不适用时才判 `R9-A` 完成。
- `R10`：阻塞处理：先在最小测试范围内消除阻塞并重验。
...
4. 输出分支清单与测试映射（`R4`）；若有额外同分支测试，记录其必要性依据（`R12`）。
5. 按 `R8` 执行死代码分析并记录结果。
6. 按实现规则实现或扩展测试（`R2`、`R4`、`R5`、`R6`、`R7`、`R3`、`R11`、`R12`、执行边界）。
7. 按 `R12` 执行必要性自检，删除冗余 mock/stub/assertion、单次使用局部变量和覆盖等价测试方法。
8. 按 `R11` 执行第一次硬门禁扫描（早期快速失败）并修复所有命中。
9. 按分层顺序运行验证命令（目标测试 -> 目标模块门禁 -> 必要时回退门禁）并迭代。
10. 按 `R11` 执行第二次硬门禁扫描（最终发布门禁）并确保干净。
...
4. `R11` 硬门禁扫描（必须干净，在工作流第 8 步和第 10 步执行）：
```bash
bash -lc '
...
if rg -n "$BOOLEAN_ASSERTION_BAN_REGEX" <ResolvedTestFileSet>; then
  echo "[R11] forbidden boolean assertion found"
  exit 1
fi'
```
...
./mvnw <TestModuleFlags> -DskipITs -Dspotless.skip=true -Dtest=<ResolvedTestClass> -Dsurefire.failIfNoSpecifiedTests=false test
```
- 执行第 4 项 `R11` 硬门禁扫描命令，并记录结果（最终发布门禁）。
...
1. 目标与约束（映射到 `R1-R12`）
2. 状态（由 `R9` 判定）+ 一行原因
3. 计划与实现（包含模块推导证据、分支映射结果及任何 `R12` 例外理由）
4. 死代码与覆盖率结果（至少包含：目标类覆盖率数值、死代码定位与原因）
5. 验证与质量门禁证据（至少包含：关键命令、退出码、`R11` 扫描结果）
