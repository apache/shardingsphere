...
## 开发理念
 ...
 - **用心** 在设计、开发以及重构时反复推敲，保持对代码的责任心和敬畏心，以工匠精神持续雕琢。
 - **可读** 以代码可读性为首要目标，代码无歧义，通过阅读而非调试手段即可体现代码原意。
 - **整洁** 追求整洁和优雅的代码，认同《重构》和《代码整洁之道》的理念。
 - **一致** 代码风格、命名以及使用方式保持强一致。
 - **精简** 追求极简代码，以最少的代码表达最正确的意思。高度复用，无重复代码和配置。及时删除无用代码。
 - **抽象** 清晰合理的划分层次，提炼概念。保持方法、类、包以及模块等在同一抽象层级。
 - **极致** 保证任何一行代码、任何一个字母、任何一个空格都有其存在价值。
