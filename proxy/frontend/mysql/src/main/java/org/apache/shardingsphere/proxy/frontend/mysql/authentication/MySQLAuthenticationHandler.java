...
import org.apache.shardingsphere.proxy.frontend.mysql.authentication.authenticator.MySQLAuthenticatorFactoryEngine;
...
public final class MySQLAuthenticationHandler {
    ...
    public Authenticator getAuthenticator(final AuthorityRule rule, final ShardingSphereUser user) {
        return new MySQLAuthenticatorFactoryEngine().createAuthenticator(rule.getAuthenticatorType(user), rule);
    }
}
