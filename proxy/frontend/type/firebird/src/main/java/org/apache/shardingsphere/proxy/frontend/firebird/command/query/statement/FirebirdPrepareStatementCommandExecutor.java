...
public final class FirebirdPrepareStatementCommandExecutor implements CommandExecutor {
    ...
    private int processInsertStatement(InsertStatementContext sqlStatementContext, MetaDataContexts metaDataContexts, ByteBuf buffer, List<FirebirdSQLInfoPacketType> requestedItems) {
    ...
    private Collection<String> getTableNames(SQLStatementContext sqlStatementContext) {
        if (sqlStatementContext instanceof TableAvailable) {
            return ((TableAvailable) sqlStatementContext).getTablesContext().getTableNames();
        }
    ...
    private boolean processExpr(ExpressionSegment expr, List<ColumnSegment> affectedColumns) {
        if (expr instanceof BinaryOperationExpression) {
            ...
            if (isParameter) {
                ...
                if (left instanceof ColumnSegment) {
                    affectedColumns.add((ColumnSegment) left);
                }
            }
        }
    ...
    private void processExpressionProjection(ExpressionProjection expr, ByteBuf buffer, List<FirebirdSQLInfoPacketType> requestedItems, int columnCount) {
    ...
    private void processCustomColumn(String tableName, String columnName, Optional<IdentifierValue> columnAlias, int dataType, ByteBuf buffer, List<FirebirdSQLInfoPacketType> requestedItems, int columnCount) {
    ...
    private void processColumn(ByteBuf buffer, List<FirebirdSQLInfoPacketType> requestedItems, ShardingSphereTable table, ShardingSphereColumn column, Optional<IdentifierValue> tableAlias, Optional<IdentifierValue> columnAlias, int idx) {
