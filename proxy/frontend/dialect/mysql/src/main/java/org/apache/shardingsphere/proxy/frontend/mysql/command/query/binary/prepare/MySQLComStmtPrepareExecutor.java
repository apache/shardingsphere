...
public final class MySQLComStmtPrepareExecutor implements CommandExecutor {
    ...
    public Collection<DatabasePacket> execute() {
        ...
        Collection<DatabasePacket> result = createPackets(sqlStatementContext, statementId, serverPreparedStatement);
        // The statement is registered only after the packets were built, so a failed prepare (e.g. backend metadata resolution)
        // leaves no unreachable entry behind: the client cannot close a statement whose ID never reached it.
    ...
    private ShardingSphereSchema getSchema(final SQLStatementContext sqlStatementContext) {
        ...
        return sqlStatementContext.getTablesContext().getSchemaName().map(database::getSchema)
                .orElseGet(() -> database.findDefaultSchema().orElse(null));
    }
