    ...
    private void processReturnValues(final SQLStatementContext sqlStatementContext, final MetaDataContexts metaDataContexts, final Collection<FirebirdReturnColumnPacket> describeColumns,
        ...
        for (Projection each : projections) {
            if (each instanceof ColumnProjection) {
                ...
                if (table == null) {
                    table = metaDataContexts.getMetaData().getDatabase(databaseName).getSchema("system_tables").getTable(tableName.isEmpty() ? getTableNames(sqlStatementContext).iterator().next() : tableName);
                }
