...
/**
 * Fuzzy encrypt algorithm.
 */
@Getter
public final class FuzzyChineseEncryptAlgorithm implements EncryptAlgorithm<String, String> {
    ...
    public static HashMap<String,Integer> DICT_SEARCH;

    @Override
    public void init(final Properties props) {
        char[] chars = DICT.toCharArray();
        for (int i = 0; i < chars.length; i++) {
            DICT_SEARCH.put(String.valueOf(chars[i]), i);
        }
        this.props = props;
        if (props == null) {
            return;
        ...
        try {
            mask = Integer.parseInt(props.getProperty("mask"));
        } catch (NumberFormatException e) {
            // do nothing
        }
    }
