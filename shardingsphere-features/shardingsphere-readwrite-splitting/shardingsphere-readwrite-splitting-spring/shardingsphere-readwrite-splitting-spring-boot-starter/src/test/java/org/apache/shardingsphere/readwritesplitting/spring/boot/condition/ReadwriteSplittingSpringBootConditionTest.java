...
package org.apache.shardingsphere.readwritesplitting.spring.boot.condition;
...
public final class ReadwriteSplittingSpringBootConditionTest {
    ...
    public void assertMatch() {
        MockEnvironment mockEnvironment = new MockEnvironment();
        mockEnvironment.setProperty("spring.shardingsphere.rules.readwrite-splitting.data-sources.readwrite_ds.primary-data-source-name", "primary_ds");
        ConditionContext context = mock(ConditionContext.class);
        AnnotatedTypeMetadata metadata = mock(AnnotatedTypeMetadata.class);
        when(context.getEnvironment()).thenReturn(mockEnvironment);
        ReadwriteSplittingSpringBootCondition condition = new ReadwriteSplittingSpringBootCondition();
