...
public final class ShardingInsertValuesTokenGenerator implements OptionalSQLTokenGenerator<InsertStatementContext>, RouteContextAware {
    ...
    public InsertValuesToken generateSQLToken(final InsertStatementContext insertStatementContext) {
        ...
        Iterator<Collection<DataNode>> originalDataNodesIterator = null == routeContext || routeContext.lastRouteStageContext().getRouteResult().getOriginalDataNodes().isEmpty()
                ? null : routeContext.lastRouteStageContext().getRouteResult().getOriginalDataNodes().iterator();
