...
/**
 * Sharding condition engine for replace clause.
 */
@RequiredArgsConstructor
public final class ReplaceClauseShardingConditionEngine {
     ...
     *
     * @param replaceStatementContext replace statement context
     * @param parameters SQL parameters
     * @return sharding conditions
     */
    public List<ShardingCondition> createShardingConditions(final ReplaceStatementContext replaceStatementContext, final List<Object> parameters) {
        List<ShardingCondition> result = new LinkedList<>();
        String tableName = replaceStatementContext.getSqlStatement().getTable().getTableName().getIdentifier().getValue();
        Collection<String> columnNames = getColumnNames(replaceStatementContext);
        ...
        if (generatedKey.isPresent() && generatedKey.get().isGenerated()) {
            generatedKey.get().getGeneratedValues().addAll(getGeneratedKeys(tableName, replaceStatementContext.getSqlStatement().getValueListCount()));
    ...
    private Collection<String> getColumnNames(final ReplaceStatementContext replaceStatementContext) {
        Optional<GeneratedKeyContext> generatedKey = replaceStatementContext.getGeneratedKeyContext();
        if (generatedKey.isPresent() && generatedKey.get().isGenerated()) {
            Collection<String> result = new LinkedList<>(replaceStatementContext.getColumnNames());
            result.remove(generatedKey.get().getColumnName());
            return result;
        }
        return replaceStatementContext.getColumnNames();
    }
        ...
        for (ExpressionSegment each : insertValueContext.getValueExpressions()) {
            ...
            if (shardingRule.isShardingColumn(columnName, tableName)) {
                if (each instanceof SimpleExpressionSegment) {
                    ...
                    throw new ShardingSphereException("Replace clause sharding column can't be null.");
                }
            }
        }
        return result;
