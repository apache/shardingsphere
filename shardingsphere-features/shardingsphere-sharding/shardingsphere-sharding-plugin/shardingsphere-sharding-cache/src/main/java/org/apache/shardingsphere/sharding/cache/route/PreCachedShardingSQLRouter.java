...
/**
 * Cached sharding SQL router for cache preflight.
 */
public final class PreCachedShardingSQLRouter implements SQLRouter<ShardingCacheRule> {
    ...
    public RouteContext createRouteContext(final QueryContext queryContext, final ShardingSphereDatabase database, final ShardingCacheRule rule, final ConfigurationProperties props,
        ...
        return rule.getRouteCache().get(new ShardingRouteCacheKey(queryContext.getSql(), shardingConditionParameters)).flatMap(ShardingRouteCacheValue::getCachedRouteContext)
                .orElseGet(RouteContext::new);
    }
