...
public final class ShardingDropIndexStatementValidatorTest {
    ...
    public void assertPreValidateDropIndexWhenIndexExistForPostgreSQL() {
        ...
        when(metaData.getSchemaByName("public").getAllTableNames()).thenReturn(Collections.singletonList("t_order"));
        when(metaData.getSchemaByName("public").get("t_order")).thenReturn(tableMetaData);
    ...
    public void assertPostValidateDropIndexWithSameRouteResultBroadcastTableIndexForPostgreSQL() {
        ...
        Map<String, IndexMetaData> indexes = mock(HashMap.class);
        when(tableMetaData.getIndexes()).thenReturn(indexes);
        when(metaData.getSchemaByName("public").getAllTableNames()).thenReturn(Collections.singletonList("t_config"));
        when(metaData.getSchemaByName("public").get("t_config")).thenReturn(tableMetaData);
