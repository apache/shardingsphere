...
public final class ShardingDropIndexStatementValidatorTest {
    ...
    public void assertPreValidateDropIndexWhenIndexExistForPostgreSQL() {
        ...
        when(database.getDatabaseMetaData().getSchemas().get("public").getAllTableNames()).thenReturn(Collections.singletonList("t_order"));
        when(database.getDatabaseMetaData().getSchemas().get("public").get("t_order")).thenReturn(tableMetaData);
    ...
    public void assertPostValidateDropIndexWithSameRouteResultBroadcastTableIndexForPostgreSQL() {
        ...
        Map<String, IndexMetaData> indexes = mock(HashMap.class);
        when(tableMetaData.getIndexes()).thenReturn(indexes);
        when(database.getDatabaseMetaData().getSchemas().get("public").getAllTableNames()).thenReturn(Collections.singletonList("t_config"));
        when(database.getDatabaseMetaData().getSchemas().get("public").get("t_config")).thenReturn(tableMetaData);
