...
public final class DumperFactory {
     ...
     * @return log dumper
     */
    public static IncrementalDumper newInstanceLogDumper(final DumperConfiguration dumperConfig, final Position<?> position) {
     ...
     */
    @SneakyThrows(ReflectiveOperationException.class)
    public static IncrementalDumper newInstanceLogDumper(final String databaseType, final DumperConfiguration dumperConfig, final Position<?> position) {
        ScalingEntry scalingEntry = ScalingEntryLoader.getInstance(databaseType);
        return scalingEntry.getIncrementalDumperClass().getConstructor(DumperConfiguration.class, Position.class).newInstance(dumperConfig, position);
    }
}
