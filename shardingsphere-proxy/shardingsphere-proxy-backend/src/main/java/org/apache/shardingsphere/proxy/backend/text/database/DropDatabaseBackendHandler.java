...
public final class DropDatabaseBackendHandler implements TextProtocolBackendHandler {
    ...
    private static Collection<ShardingSphereRule> getRules(final String schemaName) {
        ...
        ShardingSphereDatabaseMetaData databaseMetaData = ProxyContext.getInstance().getContextManager().getMetaDataContexts().getDatabaseMetaData(schemaName);
        if (null != databaseMetaData && null != databaseMetaData.getRuleMetaData()) {
            result.addAll(databaseMetaData.getRuleMetaData().getRules());
        }
