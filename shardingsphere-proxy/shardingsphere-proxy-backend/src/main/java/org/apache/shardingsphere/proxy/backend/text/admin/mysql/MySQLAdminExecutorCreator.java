...
public final class MySQLAdminExecutorCreator implements DatabaseAdminExecutorCreator {
    ...
    private boolean isShowSpecialFunction(final SelectStatement sqlStatement, final String functionName) {
        Iterator<ProjectionSegment> projectionSegmentIterator = sqlStatement.getProjections().getProjections().iterator();
        ProjectionSegment firstProjection = projectionSegmentIterator.next();
        return !projectionSegmentIterator.hasNext() && firstProjection instanceof ExpressionProjectionSegment
