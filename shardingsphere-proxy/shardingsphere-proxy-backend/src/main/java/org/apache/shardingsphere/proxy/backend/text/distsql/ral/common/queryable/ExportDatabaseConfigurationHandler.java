...
public final class ExportDatabaseConfigurationHandler extends QueryableRALBackendHandler<ExportDatabaseConfigurationStatement, ExportDatabaseConfigurationHandler> {
    ...
    protected Collection<List<Object>> getRows(final ContextManager contextManager) {
        ...
        getDataSourcesConfig(databaseMetaData, result);
        getRuleConfigurations(databaseMetaData.getRuleMetaData().getConfigurations(), result);
    ...
    private void getDataSourcesConfig(final ShardingSphereDatabaseMetaData databaseMetaData, final StringBuilder result) {
        if (null == databaseMetaData.getResource().getDataSources() || databaseMetaData.getResource().getDataSources().isEmpty()) {
            return;
        }
        configItem(ZERO, "dataSources", result);
        for (Entry<String, DataSource> each : databaseMetaData.getResource().getDataSources().entrySet()) {
