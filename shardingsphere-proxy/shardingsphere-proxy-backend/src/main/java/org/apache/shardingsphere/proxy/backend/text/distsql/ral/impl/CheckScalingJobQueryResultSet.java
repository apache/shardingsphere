    ...
    public void init(final ShardingSphereMetaData metaData, final SQLStatement sqlStatement) {
        data = scalingAPI.dataConsistencyCheck(((CheckScalingJobStatement) sqlStatement).getJobId()).entrySet().stream()
                .map(entry -> {
                    ...
                    list.add(entry.getValue().getSourceCount());
                    list.add(entry.getValue().getTargetCount());
                    list.add(entry.getValue().isCountValid() ? 1 : 0);
                    list.add(entry.getValue().isDataValid() ? 1 : 0);
                    return list;
