...
public final class PostgreSQLComQueryExecutor implements QueryCommandExecutor {
    ...
    public Collection<DatabasePacket<?>> execute() throws SQLException {
        ...
        return responseHeader instanceof UpdateResponseHeader ? Collections.singleton(createUpdatePacket((UpdateResponseHeader) responseHeader))
                : createUpdatePackets((ClientEncodingResponseHeader) responseHeader);
    }
    ...
    private Collection<DatabasePacket<?>> createUpdatePackets(final ClientEncodingResponseHeader clientEncodingResponseHeader) {
        ...
        String name = clientEncodingResponseHeader.getClientEncoding();
        final Optional<Charset> charset = PostgreSQLCharacterSet.findByName(name);
        if (charset.isPresent()) {
            ...
            result.add(new PostgreSQLParameterStatusPacket(clientEncodingResponseHeader.getName(), clientEncodingResponseHeader.getClientEncoding()));
            return result;
        }
        throw new InvalidParameterValueException(String.format("invalid value for parameter \"clientEncoding\": \"%s\"", name));
    }
