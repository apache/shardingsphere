...
public final class MySQLMultiStatementsHandler implements TextProtocolBackendHandler {
    ...
    private ShardingSphereSQLParserEngine getSQLParserEngine() {
        ...
        return sqlParserRule.getSQLParserEngine(
                DatabaseTypeEngine.getTrunkDatabaseTypeName(metaDataContexts.getMetaData().get(connectionSession.getDatabaseName()).getProtocolType()));
    }
    ...
    private ExecutionContext createExecutionContext(final LogicSQL logicSQL) {
        SQLCheckEngine.check(logicSQL.getSqlStatementContext(), logicSQL.getParameters(),
                metaDataContexts.getMetaData().get(connectionSession.getDatabaseName()).getRuleMetaData().getRules(),
                connectionSession.getDatabaseName(), metaDataContexts.getMetaData().getDatabases(), null);
        return kernelProcessor.generateExecutionContext(logicSQL, metaDataContexts.getMetaData().get(connectionSession.getDatabaseName()),
                metaDataContexts.getMetaData().getGlobalRuleMetaData(), metaDataContexts.getMetaData().getProps());
    }
    
    @Override
    public ResponseHeader execute() throws SQLException {
        Collection<ShardingSphereRule> rules = metaDataContexts.getMetaData().get(connectionSession.getDatabaseName()).getRuleMetaData().getRules();
    ...
    private UpdateResponseHeader executeBatchedStatements(final ExecutionGroupContext<JDBCExecutionUnit> executionGroupContext) throws SQLException {
        ...
        DatabaseType databaseType = metaDataContexts.getMetaData().get(connectionSession.getDatabaseName()).getResource().getDatabaseType();
