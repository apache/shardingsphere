...
public final class MySQLComQueryPacketExecutorTest {
    ...
    public void assertExecuteMultiUpdateStatements() throws SQLException, NoSuchFieldException, IllegalAccessException {
        ...
        try (MockedStatic<ProxyContext> mockedStatic = mockStatic(ProxyContext.class)) {
            ...
            when(ProxyContext.getInstance().getContextManager().getMetaDataContexts().getMetaData().get("db_name").getResource().getDatabaseType()).thenReturn(new MySQLDatabaseType());
            when(ProxyContext.getInstance().getContextManager().getMetaDataContexts().getMetaData().get("db_name").getProtocolType()).thenReturn(new MySQLDatabaseType());
