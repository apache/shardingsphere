...
class DatabaseExecutionBackendTest {
    ...
    @Test
    void assertRefreshMetadata() throws SQLException {
        String jdbcUrl = H2RuntimeTestSupport.createJdbcUrl(tempDir, "database-execution-backend-refresh");
        ...
        DatabaseExecutionBackend backend = new DatabaseExecutionBackend(runtimeDatabases, databaseMetadataSnapshots);
    ...
    void assertRefreshMetadataWithUnconfiguredDatabase() {
        DatabaseExecutionBackend backend = new DatabaseExecutionBackend(Map.of(), new DatabaseMetadataSnapshots(Map.of()));
        assertThat(assertThrows(IllegalArgumentException.class, () -> backend.refreshMetadata("logic_db")).getMessage(), is("Database `logic_db` is not configured."));
    }
}
