...
class RuleInspectionServiceTest {
    ...
    void assertQueryEncryptRulesUsesShowAllSqlWhenTableNameIsBlank() {
        ...
        when(queryService.query(requestContext, "logic_db", "", "SHOW ENCRYPT RULES FROM logic_db")).thenReturn(List.of(Map.of("table", "orders")));
        ...
        verify(queryService).query(requestContext, "logic_db", "", "SHOW ENCRYPT RULES FROM logic_db");
    }
    ...
    void assertQueryMaskRulesUsesSpecificTableSql() {
        WorkflowProxyQueryService queryService = mock(WorkflowProxyQueryService.class);
        MCPRequestContext requestContext = mock(MCPRequestContext.class);
        when(queryService.query(requestContext, "logic_db", "", "SHOW MASK RULE orders FROM logic_db"))
        ...
        verify(queryService).query(requestContext, "logic_db", "", "SHOW MASK RULE orders FROM logic_db");
    }
    ...
    void assertQueryEncryptRulesUsesSpecificTableSql() {
        WorkflowProxyQueryService queryService = mock(WorkflowProxyQueryService.class);
        MCPRequestContext requestContext = mock(MCPRequestContext.class);
        when(queryService.query(requestContext, "logic_db", "", "SHOW ENCRYPT TABLE RULE orders FROM logic_db"))
        ...
        assertThat(actual.size(), is(1));
        verify(queryService).query(requestContext, "logic_db", "", "SHOW ENCRYPT TABLE RULE orders FROM logic_db");
    }
    ...
    void assertQueryEncryptAlgorithmsDelegatesToProxyQueryService() {
        WorkflowProxyQueryService queryService = mock(WorkflowProxyQueryService.class);
        MCPRequestContext requestContext = mock(MCPRequestContext.class);
        when(queryService.queryWithAnyDatabase(requestContext, "SHOW ENCRYPT ALGORITHM PLUGINS")).thenReturn(List.of(Map.of("type", "AES")));
        RuleInspectionService service = new RuleInspectionService(queryService);
        List<Map<String, Object>> actual = service.queryEncryptAlgorithms(requestContext);
        assertThat(actual.size(), is(1));
        verify(queryService).queryWithAnyDatabase(requestContext, "SHOW ENCRYPT ALGORITHM PLUGINS");
    }
    ...
    void assertQueryMaskAlgorithmsDelegatesToProxyQueryService() {
        WorkflowProxyQueryService queryService = mock(WorkflowProxyQueryService.class);
        MCPRequestContext requestContext = mock(MCPRequestContext.class);
        when(queryService.queryWithAnyDatabase(requestContext, "SHOW MASK ALGORITHM PLUGINS")).thenReturn(List.of(Map.of("type", "MD5")));
        RuleInspectionService service = new RuleInspectionService(queryService);
        List<Map<String, Object>> actual = service.queryMaskAlgorithms(requestContext);
        assertThat(actual.size(), is(1));
        verify(queryService).queryWithAnyDatabase(requestContext, "SHOW MASK ALGORITHM PLUGINS");
    }
    ...
    void assertQueryMaskRulesRejectsUnsafeIdentifier() {
        RuleInspectionService service = new RuleInspectionService(mock(WorkflowProxyQueryService.class));
        Exception actual = assertThrows(RuntimeException.class, () -> service.queryMaskRules(mock(MCPRequestContext.class), "logic_db", "bad table"));
    ...
    void assertQueryEncryptRulesRejectsUnsafeIdentifier() {
        RuleInspectionService service = new RuleInspectionService(mock(WorkflowProxyQueryService.class));
        Exception actual = assertThrows(RuntimeException.class, () -> service.queryEncryptRules(mock(MCPRequestContext.class), "logic_db", "bad table"));
        assertThat(actual.getMessage(), is("table `bad table` contains unsupported characters. Only unquoted SQL identifiers are supported in V1."));
    }
}
