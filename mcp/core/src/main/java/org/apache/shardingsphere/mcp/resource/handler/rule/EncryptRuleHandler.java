 ...
 */
public final class EncryptRuleHandler implements ResourceHandler {
    
    private final RuleInspectionService ruleInspectionService = new RuleInspectionService();
    
    @Override
    public String getUriPattern() {
        return "shardingsphere://databases/{database}/encrypt-rules/{table}";
    }
    
    @Override
    public MCPResponse handle(final MCPRuntimeContext runtimeContext, final MCPUriVariables uriVariables) {
        return new MCPMetadataResponse(ruleInspectionService.queryEncryptRules(runtimeContext, uriVariables.getVariable("database"), uriVariables.getVariable("table")));
    }
}
