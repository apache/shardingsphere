...
/**
 * Tool handler for encrypt and mask workflow execution.
 */
public final class ApplyEncryptMaskRuleToolHandler implements ToolHandler {
    
    private static final MCPToolDescriptor TOOL_DESCRIPTOR = new MCPToolDescriptor("apply_encrypt_mask_rule",
            Arrays.asList(
    ...
    public ApplyEncryptMaskRuleToolHandler() {
        this(new WorkflowExecutionService());
    }
    
    ApplyEncryptMaskRuleToolHandler(final WorkflowExecutionService executionService) {
    ...
    public MCPResponse handle(final MCPRuntimeContext runtimeContext, final String sessionId, final Map<String, Object> arguments) {
        ...
        return new MCPMapResponse(executionService.apply(runtimeContext, sessionId, toolArguments.getStringArgument("plan_id"),
