...
/**
 * Handler for sequence resource URI.
 */
public final class SequenceHandler implements ResourceHandler {
    
    @Override
    public String getUriPattern() {
        return "shardingsphere://databases/{database}/schemas/{schema}/sequences/{sequence}";
    }
    ...
    public MCPResponse handle(final MCPRuntimeContext runtimeContext, final MCPUriVariables uriVariables) {
        ...
        String sequenceName = uriVariables.getVariable("sequence");
        return new MCPMetadataResponse(metadataQueryService.querySequence(databaseName, schemaName, sequenceName).map(Collections::singletonList).orElse(Collections.emptyList()));
    }
}
