...
public final class WorkflowExecutionService {
    ...
    private Map<String, Object> createPreviewNextAction(final WorkflowContextSnapshot snapshot) {
        ...
        result.put("reason", "Apply workflow artifacts only after the user approves the previewed side effects.");
        result.put("required_arguments", Map.of("plan_id", snapshot.getPlanId(), "execution_mode", resolveApprovedExecutionMode(snapshot)));
        result.put("requires_user_approval", true);
        return result;
    }
