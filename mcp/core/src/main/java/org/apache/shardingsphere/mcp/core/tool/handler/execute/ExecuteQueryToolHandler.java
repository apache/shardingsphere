...
public final class ExecuteQueryToolHandler implements MCPToolHandler<MCPDatabaseHandlerContext> {
    ...
    private void checkReadOnlyQuery(final String sql) {
        ...
        if (!SQLExecutionToolHandlerSupport.isReadOnlyStatement(classificationResult.getStatementClass())) {
            throw new MCPUnsupportedException("execute_query only supports read-only QUERY and EXPLAIN_ANALYZE statements. Use execute_update for side-effecting SQL.");
        }
    }
}
