...
- 在 `mcp/features/<feature>` 新建模块，依赖 `mcp/api`、service 级 handler 依赖 `mcp/core`、database metadata 或 execution handler 依赖 `mcp/database`、需要 workflow 能力时依赖 `mcp/workflow`，再加必要的领域模块；不要依赖 `mcp/bootstrap`
...
- 对外新增 tool 时，实现 `MCPToolHandler<T extends MCPHandlerContext>` 并声明所需 context type，同时提供唯一的 `MCPToolDescriptor`
- 对外新增 resource 时，实现 `MCPResourceHandler<T extends MCPHandlerContext>` 并声明所需 context type，同时提供唯一的 URI pattern
...
- tool 名和 resource URI pattern 必须在全局范围内保持唯一；重复 handler 会在启动期校验时被拒绝
