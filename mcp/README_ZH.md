...
encrypt 和 mask 在 MCP 里不是 bootstrap 内硬编码的特殊功能，而是通过 MCP contribution provider SPI 贡献到 MCP runtime 的可插拔 feature。
...
- 对外新增 tool 时，按能力选择最窄的 handler 契约：`ServerToolHandler`、`DatabaseToolHandler` 或 `WorkflowToolHandler`，并提供唯一的 `MCPToolDescriptor`
- 对外新增 resource 时，按能力选择最窄的 handler 契约：`ServerResourceHandler` 或 `DatabaseResourceHandler`，并提供唯一的 URI pattern
- 实现一个 `MCPContributionProvider`，通过 `getToolContributions()` 和 `getResourceContributions()` 返回该 feature 自己暴露的 contributions
- 在 `src/main/resources/META-INF/services/` 下注册 `org.apache.shardingsphere.mcp.api.spi.MCPContributionProvider`
- feature URI 统一使用 `shardingsphere://features/<feature>/...` 命名空间，避免和公共 metadata path 混用
- `mcp/core` 会通过 `ShardingSphereServiceLoader` 自动发现 contribution provider，展开并校验其中的 contributions，`mcp/bootstrap` 只负责把最终结果发布到协议层
- tool 名和 resource URI pattern 必须在全局范围内保持唯一；重复 contribution 会在启动期校验时被拒绝
