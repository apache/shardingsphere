...
- 固定 resource descriptor 可以在已知 raw content size 时声明官方 MCP `size`；`size` 必须是 Java `long` 范围内的非负整数；resource template 不能声明 `size`。
...
- 对外新增 tool 时，实现 `MCPToolHandler<T extends MCPHandlerContext>` 并声明所需 context type，同时在 `META-INF/shardingsphere-mcp/mcp-descriptors` 下添加 canonical descriptor
- 对外新增 resource 时，实现 `MCPResourceHandler<T extends MCPHandlerContext>` 并声明所需 context type，返回的 descriptor 必须解析到同一份 canonical YAML metadata
