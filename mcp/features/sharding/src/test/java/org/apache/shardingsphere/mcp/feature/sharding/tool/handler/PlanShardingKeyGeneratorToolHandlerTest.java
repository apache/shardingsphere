...
class PlanShardingKeyGeneratorToolHandlerTest {
    
    @Test
    void assertHandle() {
        try (
                MockedConstruction<ShardingWorkflowPlanningService> mocked = mockConstruction(ShardingWorkflowPlanningService.class,
                        (mock, context) -> when(mock.planKeyGenerator(any(), any(), any())).thenReturn(ShardingWorkflowToolHandlerTestFixture.createSnapshot(
                                ShardingFeatureDefinition.KEY_GENERATOR_WORKFLOW_KIND,
                                "CREATE SHARDING KEY GENERATOR `snowflake_generator`(TYPE(NAME='snowflake'))")))) {
            ...
            new PlanShardingKeyGeneratorToolHandler().handle(fixture.requestContext(), Map.of(
                    "database", "logic_db", "key_generator", "snowflake_generator", "key_generator_type", "SNOWFLAKE",
