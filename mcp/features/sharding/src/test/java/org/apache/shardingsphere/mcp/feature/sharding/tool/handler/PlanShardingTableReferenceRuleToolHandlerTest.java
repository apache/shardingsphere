...
class PlanShardingTableReferenceRuleToolHandlerTest {
    
    @Test
    void assertHandle() {
        try (
                MockedConstruction<ShardingWorkflowPlanningService> mocked = mockConstruction(ShardingWorkflowPlanningService.class,
                        (mock, context) -> when(mock.planTableReferenceRule(any(), any(), any())).thenReturn(ShardingWorkflowToolHandlerTestFixture.createSnapshot(
                                ShardingFeatureDefinition.TABLE_REFERENCE_WORKFLOW_KIND,
                                "CREATE SHARDING TABLE REFERENCE RULE `ref_rule`(`t_order`, `t_order_item`)")))) {
