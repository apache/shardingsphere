...
class PlanDefaultShadowAlgorithmToolHandlerTest {
    
    @Test
    void assertHandle() {
        try (
                MockedConstruction<ShadowWorkflowPlanningService> mocked = mockConstruction(ShadowWorkflowPlanningService.class,
                        (mock, context) -> when(mock.planDefaultAlgorithm(any(), any(), any())).thenReturn(createSnapshot()))) {
    ...
    private WorkflowContextSnapshot createSnapshot() {
        ...
        result.setWorkflowKind(WorkflowKind.valueOf(ShadowFeatureDefinition.DEFAULT_ALGORITHM_WORKFLOW_KIND.getValue()));
