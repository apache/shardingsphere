...
class WorkflowDistSQLQueryUtilsTest {
    ...
    void assertQueryRuleRowsWithUnsupportedDistSQL() {
        MCPFeatureQueryFacade queryFacade = mock(MCPFeatureQueryFacade.class);
        MCPQueryFailedException expectedCause = new MCPDatabaseQueryFailedException(MCPJDBCErrorCategory.SYNTAX, new SQLException("syntax error", "42601"));
        when(queryFacade.query("logic_db", "SHOW MASK RULES")).thenThrow(expectedCause);
        MCPUnsupportedException actual = assertThrows(MCPUnsupportedException.class,
                () -> WorkflowDistSQLQueryUtils.queryRuleRows(queryFacade, "logic_db", "SHOW MASK RULES"));
        assertThat(actual.getMessage(), is("The configured runtime does not support rule inspection DistSQL."));
        assertThat(actual.getCause(), sameInstance(expectedCause));
    }
